...
async function foundation(symbol) {
  const now=new Date(), p5=new Date(Date.now()-5*365.25*86400000);
  ...
  const [quote,annual,quarterly,historyResult]=await Promise.allSettled([
    ...
    verifiedHistory(symbol,{interval:'1d',days:3650,minBars:60}),
  ]);
  ...
  const pr=(history?.rows||[]).map(r=>({date:iso(r.date),open:num(r.o),high:num(r.h),low:num(r.l),close:num(r.c),volume:num(r.v)})).filter(r=>r.date&&r.close!=null);
  const sd=q?.summaryDetail||{}, ks=q?.defaultKeyStatistics||{}, fd=q?.financialData||{};
  const market={price:num(sd.regularMarketPrice)??pr.at(-1)?.close??null,currency:q?.price?.currency||null,exchange:q?.price?.exchangeName||null,marketCap:num(sd.marketCap)??num(ks.marketCap)??null,high52:num(sd.fiftyTwoWeekHigh),low52:num(sd.fiftyTwoWeekLow),beta:num(ks.beta),averageVolume:num(sd.averageVolume),trailingPE:num(sd.trailingPE),forwardPE:num(sd.forwardPE),priceToBook:num(ks.priceToBook),enterpriseValue:num(ks.enterpriseValue),sharesOutstanding:num(ks.sharesOutstanding),roe:num(fd.returnOnEquity)!=null?num(fd.returnOnEquity)*100:null,roa:num(fd.returnOnAssets)!=null?num(fd.returnOnAssets)*100:null,debtToEquity:num(fd.debtToEquity)!=null?num(fd.debtToEquity)/100:null,revenueGrowth:num(fd.revenueGrowth)!=null?num(fd.revenueGrowth)*100:null,earningsGrowth:num(fd.earningsGrowth)!=null?num(fd.earningsGrowth)*100:null};
  const warnings=[]; if(!q)warnings.push('Quote/fundamental summary unavailable.'); if(!ar.length)warnings.push('Annual history unavailable.'); if(!qr.length)warnings.push('Quarterly history unavailable.'); if(!pr.length)warnings.push('Verified daily price history unavailable.'); else if(pr.length<252)warnings.push(`Only ${pr.length} daily rows returned; 1Y history is incomplete.`); if(annual.status==='rejected')warnings.push(`Annual provider error: ${annual.reason?.message||annual.reason}`); if(quarterly.status==='rejected')warnings.push(`Quarterly provider error: ${quarterly.reason?.message||quarterly.reason}`); if(historyResult.status==='rejected')warnings.push(`Verified history provider error: ${historyResult.reason?.message||historyResult.reason}`);
  return {success:true,symbol:symbol.replace(/\.NS$|\.BO$/i,''),yahooSymbol:symbol,generatedAt:new Date().toISOString(),dataPolicy:{fakeDataAllowed:false,zeroFabrication:true,providerEstimatesAllowed:true,missingValueRepresentation:null,rule:'Only provider-reported values or deterministic calculations from provider-reported inputs are returned. Missing values remain null.'},market,profile:{name:q?.quoteType?.longName||q?.quoteType?.shortName||symbol,sector:q?.assetProfile?.sector||null,industry:q?.assetProfile?.industry||null,country:q?.assetProfile?.country||null},ownership:{insidersPct:num(ks.heldPercentInsiders)!=null?num(ks.heldPercentInsiders)*100:null,institutionsPct:num(ks.heldPercentInstitutions)!=null?num(ks.heldPercentInstitutions)*100:null,majorHolders:q?.majorHoldersBreakdown||null,institutionOwnership:q?.institutionOwnership?.ownershipList||[],insiderHolders:q?.insiderHolders?.holders||[],insiderTransactions:q?.insiderTransactions?.transactions||[]},history:{priceDaily:pr,annual:ar,quarterly:qr,coverage:{annual:coverage(ar,annualFields),quarterly:coverage(qr,quarterlyFields),dailyPriceRows:pr.length,dailyPriceYears:Number((pr.length/252).toFixed(1)),dailyHistorySource:history?.source||null,dailyHistoryVerified:Boolean(history?.verified)}},provenance:{provider:'Yahoo Finance via verifiedHistory market-data-provider',marketAsOf:pr.at(-1)?.date||null,fetchedAt:new Date().toISOString(),annualPeriod:'5Y requested',quarterlyPeriod:'5Y requested',pricePeriod:'10Y requested',status:history?.verified?'PROVIDER_DATA_ONLY':'UNAVAILABLE'},warnings};
}
