(() => {
  ...
  window.fetch = async (input, init = {}) => {
    ...
    if (url.origin !== window.location.origin || !url.pathname.startsWith('/api/') || PUBLIC_API.has(url.pathname)) return originalFetch(input, init);
  ...
  async function boot() {
    try {
      const publishableKey = window.__STOCKSAMJHO_CLERK_PUBLISHABLE_KEY__;
