// Decision layer: converts verified research inputs into a horizon-specific research stance.
// It never fabricates missing data and never bypasses the production BUY/SELL gate.
// IMPORTANT: technical setup, investment research and execution authority remain separate.

const n = (v) => (typeof v === 'number' && Number.isFinite(v) ? v : null);
const round = (v, d = 1) => n(v) == null ? null : Number(v.toFixed(d));

export const ACTIONABILITY_POLICY_VERSION = '1.1';
...
function horizonState(analysis, horizon) {
  return String(analysis?.horizonFreshness?.[horizon]?.state || '').toUpperCase();
}
...
function technicalState(t) {
  ...
  const atr = n(t?.atr);
  const vwap = n(t?.vwap ?? t?.periodVwap);
  ...
  return {
    ...
    canonicalEvidence: {
      ...
      setup: t?.setup ?? null,
      breakout: t?.breakout ?? null,
      breakdown: t?.breakdown ?? null,
      breakoutLifecycle: t?.breakoutLifecycle ?? null,
      supportResistance: t?.supportResistance ?? null,
      structure: t?.structure ?? null,
      technicalConfidence: t?.technicalConfidence ?? null,
      confidenceBasis: t?.confidenceBasis ?? null,
      riskEvidence: t?.breakoutLifecycle?.riskEvidence ?? null,
      provenance: t?.provenance ?? null,
    },
  };
}

function buildSwing(t, confidence, freshnessState) {
  ...
  if (freshnessState !== 'FRESH') {
    return {
      action: 'WAIT — CURRENTNESS / SESSION VALIDATION',
      status: 'UNKNOWN',
      entry: null,
      breakout: null,
      stopLoss: null,
      target1: null,
      target2: null,
      reasons: ['Swing currentness is not established by the explicit exchange/session freshness contract.'],
      trigger: 'Establish a fresh completed exchange-session observation before evaluating a new swing setup.',
    };
  }
...
function buildShortTerm(t, freshnessState) {
  if (freshnessState !== 'FRESH') return {
    action: 'UNKNOWN — CURRENTNESS REQUIRED',
    status: 'UNKNOWN',
    trigger: 'Short-Term stance requires an explicit currentness/expiry contract; daily evidence alone is insufficient.',
    invalidation: null,
    reasons: ['Short-Term currentness is not established.'],
  };
...
function buildIntraday(analysis) {
  const freshnessState = horizonState(analysis, 'INTRADAY');
  if (freshnessState !== 'FRESH' || String(analysis?.technical?.provenance?.timeframe || '').toLowerCase() === '1d') return {
    action: 'UNKNOWN — INTRADAY DATA NOT ESTABLISHED',
    status: 'UNKNOWN',
    reason: 'Intraday authority/currentness is not implemented by the available daily evidence path.',
    productionEligible: false,
  };
  return {
    action: 'MANUAL INTRADAY REVIEW',
    status: 'FRESH — MANUAL REVIEW ONLY',
    reason: 'Intraday evidence is explicitly supplied, but execution authority remains disabled.',
    productionEligible: false,
  };
}
...
function buildLongTerm(analysis, confidence, coverage) {
  ...
  const valuationAuthority = analysis?.valuationEvidenceAuthority || {};
  const blockers = [];
  ...
  if (valuationAuthority.eligibleForInvestmentReadiness !== true || valuationAuthority.valuationEvidenceStatus !== 'VERIFIED') blockers.push('Verified valuation evidence is unavailable; a framework fair value cannot by itself establish investment readiness.');
...
export function buildActionability(analysis, trading) {
  ...
  const swing = buildSwing(t, confidence, horizonState(analysis, 'SWING'));
  const shortTerm = buildShortTerm(t, horizonState(analysis, 'SHORT_TERM'));
  const longTerm = buildLongTerm(analysis, confidence, coverage);
  const intraday = buildIntraday(analysis);
  ...
  return {
    ...
    horizons: { longTerm, swing, shortTerm, intraday },
    valuation: {
      ...
      evidenceStatus: analysis?.valuationEvidenceAuthority?.valuationEvidenceStatus || 'UNKNOWN',
      eligibleForInvestmentReadiness: analysis?.valuationEvidenceAuthority?.eligibleForInvestmentReadiness === true,
    },
    evidence: {
      ...
      technicalProvenance: t?.provenance ?? null,
      technicalRiskEvidence: t?.breakoutLifecycle?.riskEvidence ?? null,
      horizonFreshness: analysis?.horizonFreshness || null,
    },
  };
}
