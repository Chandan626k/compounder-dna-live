...
function mapFreshness(value) {
  ...
  if (normalized === 'FRESH' || normalized === 'RECENT') return 'FRESH';
  if (normalized === 'STALE') return 'STALE';
  if (normalized === 'VERY_STALE' || normalized === 'EXPIRED') return 'EXPIRED';
  return 'UNKNOWN';
}

function horizonFreshness(analysis, horizon) {
  const explicit = analysis?.horizonFreshness?.[horizon] ?? analysis?.[horizon]?.freshness;
...
function buildLongTermState(analysis) {
  ...
  const freshness = horizonFreshness(analysis, 'longTerm');
  if (freshness === 'STALE' || freshness === 'EXPIRED') limitations.push('Long-Term freshness is not current enough for a fresh classification.');

  if (evidence.status.status === 'INSUFFICIENT_EVIDENCE' || !evidence.annualHistory || freshness === 'STALE' || freshness === 'EXPIRED') {
    return {
...
function buildSwingState(analysis) {
  ...
  if (freshness === 'STALE' || freshness === 'EXPIRED') limitations.push('Swing setup evidence is stale for current setup readiness.');
  ...
  if (freshness === 'STALE' || freshness === 'EXPIRED') {
    return { state: 'NO_CLEAN_SETUP', ...base, negativeEvidence: ['stale current setup evidence'] };
  }
...
export function buildMultiHorizonIntelligence(analysis) {
  ...
  return {
    longTerm: buildLongTermState(analysis),
    swing: buildSwingState(analysis),
  };
}
