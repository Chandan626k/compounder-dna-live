...
function horizonFreshness(analysis, horizon) {
  const contract = analysis?.horizonFreshnessContract?.[horizon] || analysis?.horizonFreshnessContract?.[String(horizon || '').toLowerCase()];
  if (contract && typeof contract === 'object') return resolveHorizonFreshness(contract).state;
  const explicit = analysis?.horizonFreshness?.[horizon] ?? analysis?.horizonFreshness?.[String(horizon || '').toLowerCase()] ?? analysis?.[horizon]?.freshness;
