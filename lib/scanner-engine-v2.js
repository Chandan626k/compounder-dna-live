...
export function buildScannerSetup(t,intra){
  ...
  const riskVerified=risk?.status==='VERIFIED'&&Number.isFinite(risk?.invalidationLevel)&&Number.isFinite(risk?.riskReward)&&risk.riskReward>0;
  ...
  const buyRiskAvailable=riskVerified&&direction==='UP'&&invalidation<p&&target>p&&targetEvidence;
  const sellRiskAvailable=riskVerified&&direction==='DOWN'&&invalidation>p&&target<p&&targetEvidence;
  let action='WAIT';
  if(t.bullScore>=72&&direction==='UP'&&lifecycle?.status&&['SUCCESSFUL_RETEST','CONTINUATION'].includes(lifecycle.status)&&t.trend!=='DOWNTREND'&&t.rsi!=null&&t.rsi<76&&volumeConfirmed&&atrVerified&&buyRiskAvailable&&rr>=1.5&&t.historyBars>=120&&notOverextended)action='BUY_READY';
  else if(t.bearScore>=72&&direction==='DOWN'&&lifecycle?.status&&['SUCCESSFUL_RETEST','CONTINUATION'].includes(lifecycle.status)&&t.trend==='DOWNTREND'&&t.rsi!=null&&t.rsi>24&&volumeConfirmed&&atrVerified&&sellRiskAvailable&&rr>=1.5&&t.historyBars>=120&&notOverextended)action='SELL_READY';
