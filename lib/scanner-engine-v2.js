...
export function buildScannerSetup(t,intra){
  const p=t.last, a=Number.isFinite(t.atr)?t.atr:null;
  const bs=Number.isFinite(a)&&Number.isFinite(t.e20)&&Number.isFinite(p)?Math.min(t.e20-a*.8,p-a*1.4):null;
  const ss=Number.isFinite(a)&&Number.isFinite(t.e20)&&Number.isFinite(p)?Math.max(t.e20+a*.8,p+a*1.4):null;
  const breakoutBase=t.structure?.breakout?.level||((t.prices||[]).length?Math.max(...t.prices.slice(-20)):null);
  const breakdownBase=t.structure?.breakdown?.level||((t.prices||[]).length?Math.min(...t.prices.slice(-20)):null);
  const bt=Number.isFinite(breakoutBase)?breakoutBase*1.002:null, st=Number.isFinite(breakdownBase)?breakdownBase*.998:null;
  const b1=Number.isFinite(bt)&&Number.isFinite(a)?bt+2*a:null,b2=Number.isFinite(bt)&&Number.isFinite(a)?bt+3.5*a:null,s1=Number.isFinite(st)&&Number.isFinite(a)?st-2*a:null,s2=Number.isFinite(st)&&Number.isFinite(a)?st-3.5*a:null;
  const brr=Number.isFinite(b1)&&Number.isFinite(bt)&&Number.isFinite(bs)&&bt>bs?(b1-bt)/(bt-bs):null;
  const srr=Number.isFinite(st)&&Number.isFinite(s1)&&Number.isFinite(ss)&&ss>st?(st-s1)/(ss-st):null;
  let action='WAIT';
  if(t.bullScore>=72&&t.trend!=='DOWNTREND'&&t.rsi!=null&&t.rsi<76&&t.relativeVolume!=null&&t.relativeVolume>=1.2&&brr!=null&&brr>=1.5&&t.historyBars>=120)action='BUY_READY';
  else if(t.bearScore>=72&&t.trend==='DOWNTREND'&&t.rsi!=null&&t.rsi>24&&t.relativeVolume!=null&&t.relativeVolume>=1.2&&srr!=null&&srr>=1.5&&t.historyBars>=120)action='SELL_READY';
  ...
  const relativeVolumeScore=Math.min(100,(t.relativeVolume||0)/1.5*100);
  ...
  const rrCandidates=[brr,srr].filter(Number.isFinite),rr=rrCandidates.length?Math.max(...rrCandidates):null;
  ...
  return{action,confidence:conf,readiness:Math.max(t.bullScore,t.bearScore),strategyScore,riskRewardScore:rrScore,tradeHorizon:action.includes('READY')?'NEXT_SESSION':action.includes('PULLBACK')||action.includes('RALLY')?'SWING_WATCH':'NO_SETUP',buy:{trigger:bt==null?null:+bt.toFixed(2),stop:bs==null?null:+bs.toFixed(2),target1:b1==null?null:+b1.toFixed(2),target2:b2==null?null:+b2.toFixed(2),riskReward:brr==null?null:+brr.toFixed(2)},sell:{trigger:st==null?null:+st.toFixed(2),stop:ss==null?null:+ss.toFixed(2),target1:s1==null?null:+s1.toFixed(2),target2:s2==null?null:+s2.toFixed(2),riskReward:srr==null?null:+srr.toFixed(2)},gates:{historyVerified:t.historyBars>=120,volumeConfirmed:t.relativeVolume!=null&&t.relativeVolume>=1.2,riskRewardConfirmed:rr!=null&&rr>=1.5,multiTimeframeChecked:Boolean(intra)}}
}
