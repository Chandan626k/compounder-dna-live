...
export function mergeStatementEvidence(financials,evidence) {
  ...
  const promoted=(field,item)=>{
    ...
    if(!record || record.value !== value){
      const row=item?.row||{};
      record=createCanonicalEvidence({source:'YAHOO_FINANCE',sourceKey:item?.key,issuer:row.issuer??row.companyName??null,ticker:evidence?.ticker??null,value,reportingDate:item?.date,reportingPeriod:row.reportingPeriod??item?.date?.slice(0,10)??null,periodType:row.periodType??row.requestedPeriod??'ANNUAL',statementScope:row.statementScope??row.scope??null,unit:row.unit??row.units??null,currency:row.currency??null,reportedOrDerived:'REPORTED',status:item?.status,retrievedAt:evidence?.fetchedAt});
      canonical.byId[record.evidenceId]=record;
