...
const canonicalizeEvidence = (evidence, ticker, fetchedAt) => {
  ...
  for (const fields of Object.values(evidence || {})) for (const [field,item] of Object.entries(fields || {})) {
    ...
    if (periodType !== '12M') continue;
    const record=createCanonicalEvidence({ source:'YAHOO_FINANCE', sourceKey:item.key, issuer:row.issuer ?? row.companyName ?? null, ticker, value:item.value, reportingDate:item.date, reportingPeriod:row.reportingPeriod ?? item.date.slice(0,10), periodType, statementScope:row.statementScope ?? row.scope ?? null, unit:row.unit ?? row.units ?? null, currency:row.currency ?? null, reportedOrDerived:'REPORTED', status:item.status, retrievedAt:fetchedAt });
...
export function mergeStatementEvidence(financials,evidence) {
  ...
  const guarded=(name,fields,formula)=>{
    if(!allCompatible(canonical,fields)){ derived[name]=null; return; }
    const values=Object.fromEntries(fields.map((field)=>[field,statementValue(field)]));
    derived[name]=formula(values);
  };
  const derivedEvidence={...(financials?.derivedEvidence||{})};
  const setDerivedEvidence=(name,fields)=>{
    const ids=fields.map((field)=>canonical.fields[field]).filter(Boolean);
    if(ids.length===fields.length) derivedEvidence[name]={reportedOrDerived:'DERIVED',inputEvidenceIds:ids,calculation:'APPLICATION_DERIVED',status:'VERIFIED_COMPATIBLE_STATEMENT_INPUTS'};
  ...
  return {...financials,current,derived,derivedEvidence,evidence:canonical,rawAvailability,statementEvidence:evidence,sourceNote:`${financials?.sourceNote||''} Separate annual financials, balance-sheet and cash-flow requests are used as an evidence recovery path.`.trim()};
}