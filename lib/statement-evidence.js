  ...
  for (const [section, rows] of Object.entries(rowsBySection)) {
    ...
    for (const [field, keys] of Object.entries(fieldMap)) {
      ...
      for(const row of rows || []) {
        ...
        const record=canonicalRecordFromItem({status:'PROVIDER_RETURNED',value:num(row[key]),key,date:row.date,row},ticker,fetchedAt); if(!record) continue;
...
export function mergeStatementEvidence(financials,evidence) {
  ...
  const promoted=(field,item)=>{
    const value=valueOf(item); if(value==null){if(!(field in current))current[field]=null;return;} if(current[field]!=null)return;
    const candidateId=canonical.fields[field]||canonical.observations[field]; let id=candidateId; let record=id?canonical.byId?.[id]||null:null;
    if(!record||record.value!==value){const row=item?.row||{},base=record||{},periodType=row.periodType==null?null:String(row.periodType).toUpperCase();if(periodType!=='12M'){current[field]=null;return;}record=createCanonicalEvidence({source:base.source??'YAHOO_FINANCE',sourceKey:item?.key??base.sourceKey,issuer:row.issuer??row.companyName??base.issuer??null,ticker:evidence?.ticker??base.ticker??null,value,reportingDate:item?.date??base.reportingDate,reportingPeriod:row.reportingPeriod??base.reportingPeriod??item?.date?.slice(0,10)??null,periodType,statementScope:row.statementScope??row.scope??base.statementScope??null,unit:row.unit??row.units??base.unit??null,currency:row.currency??base.currency??null,reportedOrDerived:base.reportedOrDerived??'REPORTED',status:item?.status??base.status,retrievedAt:evidence?.fetchedAt??base.retrievedAt,documentType:base.documentType,documentTitle:base.documentTitle,sourceUrl:base.sourceUrl,page:base.page,table:base.table,documentVersion:base.documentVersion,extractionMethod:base.extractionMethod});canonical.byId[record.evidenceId]=record;id=record.evidenceId;}
    if(!id||!record||record.value!==value)return; current[field]=value; canonical.fields[field]=id;
  };
  ...
  const setDerivedEvidence=(name,fields)=>{if(!allCompatible(canonical,fields)){delete derivedEvidence[name];return;}const ids=fields.map(field=>canonical.fields[field]).filter(Boolean);if(ids.length===fields.length)derivedEvidence[name]={reportedOrDerived:'DERIVED',inputEvidenceIds:ids,calculation:'APPLICATION_DERIVED',status:'VERIFIED_COMPATIBLE_STATEMENT_INPUTS'};else delete derivedEvidence[name];};
  ...
  const historyIds=(field)=>canonical.history[field]||[];
  const findHistoryId=(field,date)=>historyIds(field).map(id=>canonical.byId[id]).find(x=>x?.reportingDate===date)?.evidenceId||null;
  const latestRevenueId=latestRevenue?findHistoryId('revenue',latestRevenue.date):null, prevRevenueId=prevRevenue?findHistoryId('revenue',prevRevenue.date):null;
  const latestEpsId=latestEps?findHistoryId('eps',latestEps.date):null, prevEpsId=prevEps?findHistoryId('eps',prevEps.date):null;
  if(latestRevenueId&&prevRevenueId)derivedEvidence.latestRevenueGrowth={reportedOrDerived:'DERIVED',inputEvidenceIds:[prevRevenueId,latestRevenueId],calculation:'YEAR_OVER_YEAR',status:'VERIFIED_12M_HISTORY'};
  if(latestEpsId&&prevEpsId)derivedEvidence.latestEPSGrowth={reportedOrDerived:'DERIVED',inputEvidenceIds:[prevEpsId,latestEpsId],calculation:'YEAR_OVER_YEAR',status:'VERIFIED_12M_HISTORY'};
