...
const fieldStatus = (rows, keys, requestStatus) => {
  ...
  return { status: 'PROVIDER_DID_NOT_RETURN', value: null, key: null, date: null, row: null };
};

const canonicalizeEvidence = (evidence, ticker, fetchedAt) => {
  const canonical = { byId: {}, fields: {} };
  for (const fields of Object.values(evidence || {})) {
    for (const [field, item] of Object.entries(fields || {})) {
      if (!item?.value || item?.status !== 'PROVIDER_RETURNED' || !item?.key || !item?.date) continue;
      const row = item.row || {};
      const record = createCanonicalEvidence({
        source: 'YAHOO_FINANCE',
        sourceKey: item.key,
        issuer: row.issuer ?? row.companyName ?? null,
        ticker,
        reportingDate: item.date,
        reportingPeriod: row.reportingPeriod ?? item.date.slice(0, 10),
        periodType: row.periodType ?? row.requestedPeriod ?? 'ANNUAL',
        statementScope: row.statementScope ?? row.scope ?? null,
...
const allCompatible = (canonical, fields) => {
  ...
  for (let i = 1; i < evidence.length; i += 1) {
    if (!areEvidenceCompatible(evidence[0], evidence[i])) return false;
  }
  return true;
};
...
export async function fetchStatementEvidence(symbol) {
  ...
  const canonicalEvidence = canonicalizeEvidence(evidence, ticker, fetchedAt);
  ...
  return {
    ...
    validation: {
      ...
      policy: 'Only provider-returned annual statement values are accepted; missing fields remain null. Financial derivations require complete evidence metadata.',
    },
  };
}
...
export function mergeStatementEvidence(financials, evidence) {
  ...
  const promoted = (field, item) => {
    ...
    const id = canonical.fields[field];
    if (id && canonical.byId?.[id]) return;
    const row = item?.row || {};
    const record = createCanonicalEvidence({
      source: 'YAHOO_FINANCE', sourceKey: item?.key, ticker: null,
      issuer: row.issuer ?? row.companyName ?? null, reportingDate: item?.date,
      reportingPeriod: row.reportingPeriod ?? item?.date?.slice(0, 10) ?? null,
      periodType: row.periodType ?? row.requestedPeriod ?? 'ANNUAL',
      statementScope: row.statementScope ?? row.scope ?? null,
      unit: row.unit ?? row.units ?? null, currency: row.currency ?? null,
      reportedOrDerived: 'REPORTED', status: item?.status, retrievedAt: evidence?.fetchedAt,
    });
    canonical.byId[record.evidenceId] = record;
    canonical.fields[field] = record.evidenceId;
  };

  promoted('totalAssets', b.totalAssets);
  promoted('totalDebt', b.totalDebt);
  promoted('cash', b.cash);
  promoted('equity', b.equity);
  promoted('currentAssets', b.currentAssets);
  promoted('currentLiabilities', b.currentLiabilities);
  promoted('workingCapital', b.workingCapital);
  promoted('ordinaryShares', b.ordinaryShares);
  promoted('revenue', i.revenue);
  promoted('ebitda', i.ebitda);
  promoted('ebit', i.ebit);
  promoted('eps', i.eps);
  promoted('netIncome', i.netIncome);
  promoted('interestExpense', i.interestExpense);
  promoted('freeCashFlow', c.freeCashFlow);
  promoted('operatingCashFlow', c.operatingCashFlow);
  promoted('investingCashFlow', c.investingCashFlow);
  promoted('financingCashFlow', c.financingCashFlow);
  promoted('changeInCash', c.changeInCash);
  promoted('capitalExpenditure', c.capitalExpenditure);
  promoted('dividends', c.dividends);
  promoted('stockBasedCompensation', c.stockBasedCompensation);

  // Statement-derived ratios are only eligible when every input observation has
  // compatible issuer/ticker/date/period/scope/unit/currency evidence.
  const guarded = (name, fields, formula) => {
    derived[name] = allCompatible(canonical, fields) ? formula() : null;
  };
