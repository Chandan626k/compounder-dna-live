...
function riskEvidence({ direction, entry, entryZone, invalidation, targetZones, status, timeframe, provenance }) {
  const validEntry = finite(entry), validInvalidation = finite(invalidation), risk = validEntry && validInvalidation ? Math.abs(entry - invalidation) : null, targets = targetZones.filter(finite);
  const positiveRewards = targets.map((target) => direction === 'UP' ? target - entry : entry - target).filter((value) => value > 0), reward = risk != null && positiveRewards.length ? Math.max(...positiveRewards) : null, rr = risk > 0 && reward != null ? reward / risk : null;
  return { entryZone: entryZone ?? (validEntry ? { center: entry } : null), invalidationLevel: validInvalidation ? invalidation : null, stopReference: validInvalidation ? { level: invalidation, basis: 'breakout/retest thesis invalidation' } : null, targetZones: targets.length ? targets : null, risk, reward, riskReward: rr, status: status === 'SUCCESSFUL_RETEST' || status === 'CONTINUATION' ? (risk > 0 && reward != null ? 'VERIFIED' : 'UNAVAILABLE') : 'UNAVAILABLE', timeframe, provenance };
}
export function calculateCanonicalBreakoutLifecycle(rows, context = {}, options = {}) {
  ...
  for (let i = Math.max(20, pivotSpan * 2 + 2); i < rows.length; i += 1) {
    ...
    if (closesBackInside) {
      ...
      active.failureIndex = i; active.failureDate = row.date; active.failureReason = active.retestIndex == null ? 'CLOSE_BACK_BELOW_BREAKOUT_LEVEL' : 'RETEST_CLOSE_BACK_ACROSS_BREAKOUT_LEVEL';
      events.push({ type: active.status, index: i, date: row.date }); continue;
    }
    if (active.retestIndex == null && touched && barsSinceBreakout <= maxRetestBars) {
      active.retestIndex = i; active.retestDate = row.date; active.retestExtreme = active.direction === 'UP' ? row.low : row.high; active.status = 'SUCCESSFUL_RETEST'; active.retestVolumeStatus = volumeStatus(relativeVolumeAt(rows, i)); events.push({ type: 'RETEST_HOLD', index: i, date: row.date }); continue;
    }
