...
function riskEvidence({ direction, entry, entryZone, invalidation, targetZones, status, timeframe, provenance, breakoutLevel, retestZone, marketStructure, supportResistance, atrAtBreakout }) {
  const validEntry = finite(entry), rawInvalidation = finite(invalidation);
  const invalidationIsDefensible = validEntry && rawInvalidation !== 0 && (direction === 'UP' ? rawInvalidation < entry : rawInvalidation > entry);
  const validInvalidation = invalidationIsDefensible ? rawInvalidation : null;
  const risk = validEntry && validInvalidation ? Math.abs(entry - validInvalidation) : null;
  ...
  return {
    ...
    stopReference: validInvalidation ? { level: validInvalidation, basis: 'retest extreme beyond breakout level' } : null,
...
export function calculateCanonicalBreakoutLifecycle(rows, context = {}, options = {}) {
  ...
  const current = rows.at(-1), atrNow = atrAt(rows, rows.length - 1), invalidationCandidate = active.retestIndex != null ? active.retestExtreme : null, targetLevels = Array.isArray(context.targetLevels) ? context.targetLevels.map((target) => typeof target === 'number' ? target : target?.price).filter(finite).filter((target, index) => { const source = context.targetLevels[index]; const type = typeof source === 'object' ? source?.type : null; return active.direction === 'UP' ? (type == null || type === 'RESISTANCE') && target > active.level : (type == null || type === 'SUPPORT') && target < active.level; }) : [], dedupedTargets = [...new Set(targetLevels.map((value) => Number(value.toFixed(6))))].sort((a, b) => active.direction === 'UP' ? a - b : b - a).slice(0, 3);
  const risk = riskEvidence({ direction: active.direction, entry: active.retestIndex != null ? active.level : null, entryZone: active.retestZone, invalidation: invalidationCandidate, targetZones: dedupedTargets, status: active.status, timeframe, provenance, breakoutLevel: active.level, retestZone: active.retestZone, marketStructure: context.marketStructure, supportResistance: context.supportResistance, atrAtBreakout: active.breakoutAtr });
