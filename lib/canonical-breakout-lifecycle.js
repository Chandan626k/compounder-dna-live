...
function isConfirmedCross(rows, index, candidate) {
  ...
  return candidate.direction === 'UP'
    ? row.close > candidate.level && previous.close <= candidate.level
    : row.close < candidate.level && previous.close >= candidate.level;
}
...
export function calculateCanonicalBreakoutLifecycle(rows, context = {}, options = {}) {
  ...
  const provenance = { symbol: context.symbol || null, source: context.source || null, retrievedAt: context.retrievedAt || null, timeframe, dataQuality: 'VERIFIED', engine: 'canonical-breakout-lifecycle' };
  ...
  const targetLevels = Array.isArray(context.targetLevels) ? context.targetLevels.map((target) => typeof target === 'number' ? target : target?.price).filter(finite).filter((target) => active.direction === 'UP' ? target > active.level : target < active.level) : [];
  const dedupedTargets = [...new Set(targetLevels.map((value) => Number(value.toFixed(6))))].sort((a, b) => active.direction === 'UP' ? a - b : b - a).slice(0, 3);
  ...
  const technicalConfidence = active.status === 'FAILED' || active.status === 'FAILED_RETEST' ? 0 : Math.round([true, active.breakoutVolumeStatus === 'CONFIRMED', active.breakoutVolumeStatus !== 'UNAVAILABLE', active.retestIndex != null, active.status === 'CONTINUATION', !overextended, risk.invalidationLevel != null].filter(Boolean).length / 7 * 100);
  ...
  return {
    ...
    confirmationEvidence: { closeBeyondLevel: true, volumeStatus: active.breakoutVolumeStatus, relativeVolume: active.breakoutRelativeVolume, atrAtBreakout: active.breakoutAtr, pivotConfirmedBeforeBreakout: true },
    ...
    events,
    noLookAhead: { pivotConfirmationBars: pivotSpan, currentObservationDate: current.date, historicalEventsUseOnlyBarsAvailableAtEvent: true },
    provenance,
  };
}
