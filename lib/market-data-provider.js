...
export async function verifiedHistory(symbol,{interval='1d',days=900,minBars=60}={}){
  ...
  if(rows.length >= minBars)return{rows,source:'Yahoo Finance chart API',status:'PRIMARY',verified:true,interval,range,bars:rows.length,latest:rows.at(-1)?.date};
  await sleep(150);
  rows=await direct(normalizedSymbol,range,interval);
  if(rows.length >= minBars)return{rows,source:'Yahoo Finance chart API',status:'PRIMARY',verified:true,interval,range,bars:rows.length,latest:rows.at(-1)?.date};
  try{
    rows=await libraryFallback(normalizedSymbol,days,interval);
    if(rows.length >= minBars)return{rows,source:'Yahoo Finance chart via yahoo-finance2',status:'FALLBACK',verified:true,interval,range,bars:rows.length,latest:rows.at(-1)?.date};
  }catch(error){
