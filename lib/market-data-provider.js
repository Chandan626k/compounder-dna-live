...
export function normalizeQuote(meta, retrievedAt = new Date().toISOString()) {
  ...
  return {
    ...
    exchangeDataDelayedBy: toNum(meta?.exchangeDataDelayedBy),
    quoteSourceName: meta?.quoteSourceName || null,
    sourceIntervalMinutes: toNum(meta?.sourceInterval),
    currency: meta?.currency || null,
    exchange: meta?.exchange || meta?.fullExchangeName || meta?.exchangeName || null,
    retrievedAt,
    status: price != null && observedAt ? 'PROVIDER_QUOTE' : 'UNAVAILABLE',
  };
}

export async function verifiedQuote(symbol) {
  const requestedSymbol = String(symbol || '').trim().toUpperCase();
  const normalizedSymbol = requestedSymbol.startsWith('^') || requestedSymbol.endsWith('.NS') || requestedSymbol.endsWith('.BO')
    ? requestedSymbol
    : `${requestedSymbol}.NS`;
  ...
  try {
    ...
    if (quote?.symbol && String(quote.symbol).toUpperCase() !== normalizedSymbol) {
      return {
        price: null, observedAt: null, ageSeconds: null, marketState: null, exchangeDataDelayedBy: null, quoteSourceName: null,
        sourceIntervalMinutes: null, currency: null, exchange: null, retrievedAt,
        status: 'UNAVAILABLE', symbol: normalizedSymbol, provider: 'Yahoo Finance quote API', error: `Provider returned mismatched symbol ${quote.symbol}`,
      };
    }
    ...
    return {
      price: null, observedAt: null, ageSeconds: null, marketState: null, exchangeDataDelayedBy: null, quoteSourceName: null,
      sourceIntervalMinutes: null, currency: null, exchange: null, retrievedAt,
      status: 'UNAVAILABLE', symbol: normalizedSymbol, provider: 'Yahoo Finance quote API',
...
function parseYahoo(json) {
  ...
  const rows = ts.map((t, i) => ({
    ...
    o: toNum(q.open?.[i]), h: toNum(q.high?.[i]), l: toNum(q.low?.[i]), c: toNum(q.close?.[i]),
    adjC: toNum(adj?.[i]), v: toNum(q.volume?.[i]),
  }));
...
async function libraryFallback(symbol, days, interval) {
  ...
  if (Array.isArray(data?.quotes)) {
    const rows = data.quotes.map(item => ({
  ...
  const rows = timestamps.map((t, i) => ({
    date: new Date(t * 1000).toISOString(), o: toNum(q.open?.[i]), h: toNum(q.high?.[i]), l: toNum(q.low?.[i]), c: toNum(q.close?.[i]), adjC: toNum(adj?.[i]), v: toNum(q.volume?.[i]),
  }));
  return { rows: validateMarketRows(rows), quote: normalizeQuote(data?.meta) };
}
