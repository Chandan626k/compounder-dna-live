...
function normalizeQuote(meta) {
  const price = toNum(meta?.regularMarketPrice);
  const marketTime = toNum(meta?.regularMarketTime);
  return {
    price,
    observedAt: marketTime != null ? new Date(marketTime * 1000).toISOString() : null,
    marketState: meta?.marketState || null,
    exchangeDataDelayedBy: toNum(meta?.exchangeDataDelayedBy),
    quoteSourceName: meta?.quoteSourceName || null,
    currency: meta?.currency || null,
    exchange: meta?.exchangeName || null,
    status: price != null && marketTime != null ? 'PROVIDER_QUOTE' : 'UNAVAILABLE',
  };
}
