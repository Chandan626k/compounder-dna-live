...
function result(rows,source,status,interval,range,retrievedAt){return{rows,source,status,verified:true,interval,range,bars:rows.length,latest:rows.at(-1)?.date,retrievedAt}}
...
export async function verifiedHistory(symbol,{interval='1d',days=900,minBars=60}={}){
  ...
  let rows=await direct(normalizedSymbol,range,interval);
  if(rows.length >= minBars)return result(rows,'Yahoo Finance chart API','PRIMARY',interval,range,new Date().toISOString());
  await sleep(150);
  rows=await direct(normalizedSymbol,range,interval);
  if(rows.length >= minBars)return result(rows,'Yahoo Finance chart API','PRIMARY',interval,range,new Date().toISOString());
  try{
    rows=await libraryFallback(normalizedSymbol,days,interval);
    if(rows.length >= minBars)return result(rows,'Yahoo Finance chart via yahoo-finance2','FALLBACK',interval,range,new Date().toISOString());
  }catch(error){
