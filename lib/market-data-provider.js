...
export async function verifiedQuote(symbol) {
  ...
  try {
    ...
    return { ...normalized, symbol: quote?.symbol || normalizedSymbol, provider: 'Yahoo Finance quote API', authority: 'NON_AUTHORITATIVE_SECONDARY' };
  } catch (error) {
    return { price: null, observedAt: null, ageSeconds: null, marketState: null, exchangeDataDelayedBy: null, quoteSourceName: null, sourceIntervalMinutes: null, currency: null, exchange: null, retrievedAt, status: 'UNAVAILABLE', freshnessStatus: 'UNAVAILABLE', symbol: normalizedSymbol, provider: 'Yahoo Finance quote API', authority: 'NON_AUTHORITATIVE_SECONDARY', error: String(error?.message || error) };
  }
}
...
function result(rows, source, status, interval, range, retrievedAt, quote) {
  return { rows, source, status, authority: 'NON_AUTHORITATIVE_SECONDARY', verified: true, payloadValidated: true, interval, range, bars: rows.length, latest: rows.at(-1)?.date, retrievedAt, currentQuote: quote || null };
}
...
export async function verifiedHistory(symbol, { interval = '1d', days = 900, minBars = 60 } = {}) {
  ...
  if (parsed.rows.length >= minBars) return result(parsed.rows, 'Yahoo Finance chart API', 'SECONDARY', interval, range, new Date().toISOString(), parsed.quote);
  await sleep(150);
  parsed = await direct(normalizedSymbol, range, interval);
  if (parsed.rows.length >= minBars) return result(parsed.rows, 'Yahoo Finance chart API', 'SECONDARY', interval, range, new Date().toISOString(), parsed.quote);
  try {
    parsed = await libraryFallback(normalizedSymbol, days, interval);
    if (parsed.rows.length >= minBars) return result(parsed.rows, 'Yahoo Finance chart via yahoo-finance2', 'SECONDARY_FALLBACK', interval, range, new Date().toISOString(), parsed.quote);
  } catch (error) {
