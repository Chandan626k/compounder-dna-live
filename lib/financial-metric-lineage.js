...
const providerEvidence = ({ sourceKey, value, retrievedAt, ticker = null, periodSemantics, qualification = 'PROVIDER_LEVEL_ONLY' }) => {
  ...
  return {
    evidenceId: hash({ source: 'YAHOO_FINANCE', ticker, sourceKey, value, retrievedAt, periodSemantics, qualification }),
    source: 'YAHOO_FINANCE', sourceKey, issuer: null, ticker, value: value ?? null, observedAt: null, retrievedAt: retrievedAt || null,
    reportingDate: null, reportingPeriod: null, periodType: null, statementScope: 'QUOTE_SUMMARY', unit: null, currency: null,
...
export function buildFinancialMetricLineage({ financials = {}, ticker = null, retrievedAt = null } = {}) {
  const metrics = {};
  const canonical = financials?.evidence?.byId || {}, canonicalFields = financials?.evidence?.fields || {};
  for (const field of STATEMENT_FIELDS) {
    const id = canonicalFields[field], record = id ? canonical[id] : null;
    metrics[field] = record
      ? { value: record.value, classification: 'CANONICAL_STATEMENT', evidenceIds: [id], status: 'TRACEABLE' }
...
export function buildValuationMetricLineage({ valuation = {}, financials = {}, ticker = null, priceSource = null, retrievedAt = null } = {}) {
  ...
  const fairValueEvidenceIds = fairValueInputs.flatMap(name => name === 'roe' ? financials?.metricLineage?.roe?.evidenceIds || [] : name === 'growthSignal' ? financials?.metricLineage?.revenue5yCagr?.evidenceIds || [] : metrics[name]?.evidenceIds || []);
  metrics.fairValue = valuation?.fairValue == null
    ...
    : { value: valuation.fairValue, classification: 'APPLICATION_DERIVED', evidenceIds: [...new Set(fairValueEvidenceIds)], inputMetrics: fairValueInputs, calculation: 'Growth-adjusted justified P/E using provider EPS/ROE/growth inputs', status: fairValueEvidenceIds.length ? 'TRACEABLE_INPUT_CHAIN' : 'INPUT_EVIDENCE_INCOMPLETE' };
