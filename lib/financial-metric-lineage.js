...
export function buildFinancialMetricLineage({ financials = {}, ticker = null, retrievedAt = null } = {}) {
  ...
  const growthInputs = ['revenue5yCagr','eps5yCagr','pat5yCagr'].filter(name => financials?.growth?.[name] != null);
  const growthEvidenceIds = growthInputs.flatMap(name => metrics[name]?.evidenceIds || []);
  if (financials?.growth?.latestRevenueGrowth != null) growthInputs.push('latestRevenueGrowth');
  if (financials?.growth?.latestEPSGrowth != null) growthInputs.push('latestEPSGrowth');
  if (financials?.ratios?.earningsGrowth != null) growthInputs.push('earningsGrowth');
  const growthSignalValue = financials?.growthSignal ?? null;
  metrics.growthSignal = growthSignalValue == null ? { value: null, classification: 'UNAVAILABLE', evidenceIds: [], inputMetrics: growthInputs, status: 'UNAVAILABLE' } : { value: growthSignalValue, classification: 'APPLICATION_DERIVED', evidenceIds: [...new Set(growthEvidenceIds)], inputMetrics: growthInputs, calculation: 'Average of available EPS CAGR, PAT CAGR and provider earnings growth candidates', status: growthEvidenceIds.length ? 'TRACEABLE_PARTIAL_INPUT_CHAIN' : 'INPUT_EVIDENCE_INCOMPLETE' };
  return metrics;
}
...
export function buildValuationMetricLineage({ valuation = {}, financials = {}, ticker = null, priceSource = null, retrievedAt = null } = {}) {
  ...
  const fairValueEvidenceIds = fairValueInputs.flatMap(name => name === 'roe' ? financials?.metricLineage?.roe?.evidenceIds || [] : name === 'growthSignal' ? financials?.metricLineage?.growthSignal?.evidenceIds || [] : metrics[name]?.evidenceIds || []);
