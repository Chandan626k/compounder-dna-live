...
const providerEvidence = ({ sourceKey, value, retrievedAt, ticker = null, periodSemantics, observedAt = null, statementScope = 'QUOTE_SUMMARY', qualification = 'PROVIDER_LEVEL_ONLY' }) => {
  if (value == null) return null;
  return { evidenceId: hash({ source: 'YAHOO_FINANCE', ticker, sourceKey, value, retrievedAt, observedAt, periodSemantics, qualification }), source: 'YAHOO_FINANCE', sourceKey, issuer: null, ticker, value, observedAt, retrievedAt: retrievedAt || null, reportingDate: null, reportingPeriod: null, periodType: null, statementScope, unit: null, currency: null, reportedOrDerived: 'REPORTED', qualification, periodSemantics };
};
...
const growthCagrLineage = (financials, field, rawKey, years) => {
  const rows=(financials?.statementEvidence?.income||[]).filter(r=>r?.periodType==='12M' && Number.isFinite(Number(r?.[rawKey]))).sort((a,b)=>new Date(a.date)-new Date(b.date));
  if(rows.length<2)return [];
  const end=rows.at(-1), targetMs=years*365.25*24*60*60*1000; let start=rows[0];
  for(const row of rows){if(new Date(end.date)-new Date(row.date)>=targetMs*0.75){start=row;break;}}
  const history=(financials?.evidence?.history?.[field]||[]).map(id=>financials.evidence.byId?.[id]).filter(Boolean);
  const startId=history.find(x=>x.reportingDate===start.date)?.evidenceId, endId=history.find(x=>x.reportingDate===end.date)?.evidenceId;
...
export function buildFinancialMetricLineage({ financials = {}, ticker = null, retrievedAt = null } = {}) {
  ...
  const growthDefs=[['revenue3yCagr','revenue','totalRevenue',3],['revenue5yCagr','revenue','totalRevenue',5],['eps3yCagr','eps','dilutedEPS',3],['eps5yCagr','eps','dilutedEPS',5],['pat3yCagr','netIncome','netIncomeFromContinuingAndDiscontinuedOperation',3],['pat5yCagr','netIncome','netIncomeFromContinuingAndDiscontinuedOperation',5]];
  for(const [metric,field,key,years] of growthDefs){const value=financials?.growth?.[metric];if(value==null)continue;const ids=growthCagrLineage(financials,field,key,years);metrics[metric]={value,classification:'APPLICATION_DERIVED',evidenceIds:ids,calculation:'ANNUAL_CAGR',status:ids.length===2?'TRACEABLE':'INPUT_EVIDENCE_INCOMPLETE'};}
  return metrics;
}
...
export function buildValuationMetricLineage({ valuation = {}, financials = {}, ticker = null, priceSource = null, retrievedAt = null } = {}) {
  ...
  for (const [metric, [sourceKey, semantics]] of Object.entries(VALUATION_PROVIDER_FIELDS)) {
    ...
    metrics[metric] = evidence ? { value, classification: metric === 'marketCap' && valuation?.marketCapStatus === 'calculated' ? 'APPLICATION_DERIVED' : 'PROVIDER_REPORTED', evidenceIds: [evidence.evidenceId], evidence, status: metric === 'marketCap' && valuation?.marketCapStatus === 'calculated' ? 'MISLABELED_PROVIDER_EVIDENCE' : 'TRACEABLE_PROVIDER_LEVEL' } : { value: null, classification: 'UNAVAILABLE', evidenceIds: [], status: 'UNAVAILABLE' };
  }
  const priceValue = priceSource?.source === 'CURRENT_PROVIDER_QUOTE' ? priceSource.currentQuotePrice : priceSource?.dailyBarClose;
  const priceObservedAt = priceSource?.source === 'CURRENT_PROVIDER_QUOTE' ? priceSource.observedAt : priceSource?.dailyBarObservedAt;
  const priceEvidence = providerEvidence({ sourceKey: priceSource?.source === 'CURRENT_PROVIDER_QUOTE' ? 'quote.regularMarketPrice' : 'chart.dailyClose', value: priceValue, ticker, retrievedAt: priceSource?.retrievedAt || retrievedAt, observedAt: priceObservedAt, periodSemantics: priceSource?.source === 'CURRENT_PROVIDER_QUOTE' ? 'provider current quote' : 'latest verified daily bar', statementScope: 'MARKET_PRICE' });
  metrics.currentPrice = priceEvidence ? { value: priceValue, classification: 'PROVIDER_MARKET_OBSERVATION', observationType: priceSource?.source === 'CURRENT_PROVIDER_QUOTE' ? 'CURRENT_QUOTE' : 'LATEST_DAILY_BAR', evidenceIds: [priceEvidence.evidenceId], evidence: priceEvidence, status: 'TRACEABLE' } : { value: valuation?.currentPrice ?? null, classification: 'UNAVAILABLE', observationType: 'UNKNOWN', evidenceIds: [], status: 'UNAVAILABLE' };
  if (valuation?.marketCapStatus === 'calculated' && valuation?.marketCap != null) metrics.marketCap = { value: valuation.marketCap, classification: 'APPLICATION_DERIVED', evidenceIds: [...new Set([...(metrics.sharesOutstanding?.evidenceIds || []), ...(metrics.currentPrice?.evidenceIds || [])])], inputMetrics: ['sharesOutstanding','currentPrice'], calculation: 'sharesOutstanding × currentPrice', status: metrics.sharesOutstanding?.evidenceIds?.length && metrics.currentPrice?.evidenceIds?.length ? 'TRACEABLE_INPUT_CHAIN' : 'INPUT_EVIDENCE_INCOMPLETE' };
  ...
  const growthEvidenceIds = growthInputs.flatMap(name => financials?.metricLineage?.[name]?.evidenceIds || metrics[name]?.evidenceIds || []);
  ...
  metrics.fairValue = valuation?.fairValue == null ? { value: null, classification: 'APPLICATION_DERIVED', evidenceIds: [], inputMetrics: fairValueInputs, status: 'UNAVAILABLE' } : { value: valuation.fairValue, classification: 'APPLICATION_DERIVED', evidenceIds: [...new Set(fairValueEvidenceIds)], inputMetrics: fairValueInputs, calculation: 'Growth-adjusted justified P/E using provider EPS/ROE/growth inputs', status: fairValueEvidenceIds.length === fairValueInputs.length ? 'TRACEABLE_INPUT_CHAIN' : 'INPUT_EVIDENCE_INCOMPLETE' };
  for (const metric of ['upsideToFairValue','marginOfSafety','valuationGap']) metrics[metric] = valuation?.[metric] == null ? { value: null, classification: 'APPLICATION_DERIVED', evidenceIds: [], inputMetrics: ['fairValue','currentPrice'], status: 'UNAVAILABLE' } : { value: valuation[metric], classification: 'APPLICATION_DERIVED', evidenceIds: [...(metrics.fairValue.evidenceIds || []), ...(metrics.currentPrice.evidenceIds || [])], inputMetrics: ['fairValue','currentPrice'], calculation: metric, status: metrics.fairValue.evidenceIds?.length && metrics.currentPrice.evidenceIds?.length ? 'TRACEABLE_INPUT_CHAIN' : 'INPUT_EVIDENCE_INCOMPLETE' };
  return metrics;
}
