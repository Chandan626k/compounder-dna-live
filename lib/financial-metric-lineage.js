...
export function buildValuationMetricLineage({ valuation = {}, financials = {}, ticker = null, priceSource = null, retrievedAt = null } = {}) {
  ...
  if (valuation?.marketCapStatus === 'calculated' && valuation?.marketCap != null) {
    metrics.marketCap = { value: valuation.marketCap, classification: 'APPLICATION_DERIVED', evidenceIds: [...new Set([...(metrics.sharesOutstanding?.evidenceIds || []), ...(metrics.currentPrice?.evidenceIds || [])])], inputMetrics: ['sharesOutstanding','currentPrice'], calculation: 'sharesOutstanding × currentPrice', status: metrics.sharesOutstanding?.evidenceIds?.length && metrics.currentPrice?.evidenceIds?.length ? 'TRACEABLE_INPUT_CHAIN' : 'INPUT_EVIDENCE_INCOMPLETE' };
  }
