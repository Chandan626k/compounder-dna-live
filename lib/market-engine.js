...
function technical(rows) {
  ...
  // 52-week range must use roughly 252 trading sessions, not the 60-session window
  // used for short-term support/resistance.
  const hi52 = Math.max(...rows.slice(-252).map((r) => r.high));
  const lo52 = Math.min(...rows.slice(-252).map((r) => r.low));
  const priceVs20 = e20 != null ? ((last / e20) - 1) * 100 : null;
  const priceVs50 = e50 != null ? ((last / e50) - 1) * 100 : null;
  const priceVs200 = e200 != null ? ((last / e200) - 1) * 100 : null;
  const emaAlignment = e20 != null && e50 != null && e200 != null
    ? (e20 > e50 && e50 > e200 ? 'BULLISH' : e20 < e50 && e50 < e200 ? 'BEARISH' : 'MIXED')
    : null;

  // Long-term trend is deliberately strict. A stock trading materially below
  // its 200-DMA cannot be classified as an uptrend merely because the shorter
  // EMAs look better. This prevents a strong fundamental score from masking a
  // genuine long-term technical downtrend.
  const trend =
    e200 != null
      ? last > e20 && e20 > e50 && e50 > e200
        ? 'STRONG UPTREND'
        : last > e50 && e50 > e200 && (priceVs200 == null || priceVs200 >= 0)
          ? 'UPTREND'
          : last < e200 || (last < e50 && e50 < e200)
            ? 'DOWNTREND'
            : 'RECOVERING / MIXED'
      : last > (s50 || last) && (percentChange(close, 63) == null || percentChange(close, 63) > 0)
        ? 'UPTREND'
        : 'RECOVERING / MIXED';

  const drawdown = hi52 > 0 ? ((last / hi52) - 1) * 100 : null;
  const position52W = hi52 > lo52 ? ((last - lo52) / (hi52 - lo52)) * 100 : null;
  ...
  return {
    ...
    volumeTrend: avgV != null && volume.at(-1) != null ? (volume.at(-1) >= avgV ? 'ABOVE AVERAGE' : 'BELOW AVERAGE') : null,
    trend,
    trendStrength: e200 != null && priceVs200 != null ? clamp(50 + priceVs200 * 2) : null,
    emaAlignment,
    priceVs20,
    priceVs50,
    priceVs200,
    ...
    distanceFrom200DMA: priceVs200,
    distanceFrom52WHigh: drawdown,
...
function buildFinancials(raw) {
  ...
  // Resolve cash before deriving net debt.  Referencing `cash` before its
  // declaration caused a production Temporal Dead Zone error and made the
  // otherwise valid Yahoo payload look like a market-data failure.
  ...
  return {
    ...
    periods: { currentMetrics: raw.trailing.some(r => r.periodType === 'TTM') ? 'TTM' : (raw.trailing.at(-1)?.periodType || null), historicalGrowth: '12M/annual only; TTM excluded from CAGR baselines' },
    rawAvailability: {
...
function buildValuation(raw, price) {
  ...
  const sharesOutstanding = value(k, 'sharesOutstanding');
  const marketCap = value(d, 'marketCap') ?? value(raw.summary?.price, 'marketCap') ?? (sharesOutstanding != null && price != null ? sharesOutstanding * price : null);
...
function scoreStock(financials, valuation, technical, quality) {
  ...
  if (technical.priceVs200 != null && technical.priceVs200 <= -20) risk += 10;
  else if (technical.priceVs200 != null && technical.priceVs200 <= -10) risk += 6;
  if (technical.drawdown != null && technical.drawdown <= -30) risk += 8;
  else if (technical.drawdown != null && technical.drawdown <= -20) risk += 4;
...
function decision(score, valuation, technical, quality) {
  ...
  if (score.overall >= 78 && score.risk < 40 && technical.trend !== 'DOWNTREND' &&
      technical.trend !== 'RECOVERING / MIXED' &&
      valuation.verdict !== 'EXPENSIVE' && valuation.verdict !== 'VERY EXPENSIVE') {
    return {
      action: 'BUY / ACCUMULATE',
      reason: ['Business and financial evidence are strong.', 'Valuation is not a major disqualifier and the long-term technical trend is confirmed.'],
    };
  }

  // Mixed/recovery conditions are not strong enough for an unrestricted BUY.
  // This is especially important when price is still below the 200-DMA.
  if (score.overall >= 68 && score.risk < 60 && technical.trend === 'RECOVERING / MIXED') {
    return {
      action: 'BUY ON WEAKNESS / HOLD',
      reason: ['Underlying business evidence is constructive.', 'Technical trend is recovering/mixed; wait for stronger price confirmation before adding aggressively.'],
    };
  }
