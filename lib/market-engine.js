...
function atr(rows, n = 14) {
  // Wilder ATR: seed with the first n true ranges, then apply Wilder's
  // recursive smoothing. This is the standard ATR definition used by
  // mainstream technical-analysis platforms and avoids the old SMA proxy.
  if (rows.length <= n) return null;
  const tr = [];
  for (let i = 1; i < rows.length; i++) {
    const x = rows[i];
    const p = rows[i - 1];
    if (![x.high, x.low, p.close].every(Number.isFinite)) return null;
    tr.push(Math.max(
      x.high - x.low,
...
function technical(rows) {
  ...
  const yearRows = clean.slice(-252);
  // A 52-week high/low requires a full 252-session window. Do not label a
  // shorter history as a 52-week range.
  const hasFull52WeekHistory = clean.length >= 252;
  const hi52 = hasFull52WeekHistory ? Math.max(...yearRows.map((r) => r.high)) : null;
  const lo52 = hasFull52WeekHistory ? Math.min(...yearRows.map((r) => r.low)) : null;
  ...
  const rangePosition = hi52 > lo52 ? ((last - lo52) / (hi52 - lo52)) * 100 : null;
  const atrPercent = aa != null && last > 0 ? (aa / last) * 100 : null;
  ...
  return {
    ...
    volumeTrend, trend,
    // Keep this field backward-compatible for the UI, but make the meaning
    // explicit: it is distance from the 200-DMA, not a statistical trend
    // strength score. A future regime engine can provide a true strength score.
    trendStrength: distanceFrom200DMA,
    atrPercent,
    last,
...
function buildDataQuality(financials, valuation, ownership, technical) {
  ...
  const checks = [
    ...
    ['P/E', valuation.trailingPE, 1.0, false],
    ['Forward P/E', valuation.forwardPE, 0.5, false],
    ['P/B', valuation.priceToBook, 0.5, false],
    // Fair value is model-dependent and may be legitimately unavailable
    // (for example, negative/unstable EPS). It must not be treated as a
    // critical data-quality failure.
    ['Fair Value', valuation.fairValue, 1.0, false],
  ...
  return {
    ...
    validation: {
      ...
      providerPayloadValidation: 'disabled; application normalization applied',
      missingDataPolicy: 'null / unavailable; no fabricated financial values',
      methodologyStatus: 'FOUNDATION — source-level freshness and cross-field anomaly checks pending',
    },
...
function scoreStock(financials, valuation, technical, quality) {
  ...
  const debtToEquity = r.debtToEquity ?? d.debtToEquityFromStatements;
  if (debtToEquity != null) {
    financialStrength += debtToEquity <= 0.25 ? 20 : debtToEquity <= 0.75 ? 10 : debtToEquity <= 1.5 ? 0 : debtToEquity <= 2.5 ? -8 : -18;
  }
  ...
  // v1.0 is intentionally a heuristic model. These weights are transparent
  // and reproducible, but are NOT empirically validated until the backtesting
  // layer is implemented. Do not present them as statistically optimized.
  const modelWeights = {
    quality: 0.22,
    growth: 0.15,
    wealth: 0.14,
  ...
  const overall = clamp(
    q * modelWeights.quality +
    growth * modelWeights.growth +
    wealth * modelWeights.wealth +
    financialStrength * modelWeights.financialStrength +
    valuationScore * modelWeights.valuation +
    technicalScore * modelWeights.technical +
    (100 - risk) * modelWeights.riskAdjusted +
    quality.confidence * modelWeights.confidence
  );

  return {
    modelVersion: 'compounder-v1.0-heuristic',
    modelStatus: 'INITIAL MODEL — SUBJECT TO BACKTESTING',
...
export async function analyze(input) {
  ...
  const response = {
    analysisVersion: '2026-08-foundation-v1',
    methodologyStatus: 'FOUNDATION — deterministic engine; heuristic scoring pending backtesting',
    stock,
    score,
