...
export function assessProviderMonetarySemantics(raw, statementEvidence = null) {
  ...
  return {
    ...
    tradingCurrency, financialCurrency, currencyStatus, evCurrencyBasis: financialCurrency, observationAt,
...
function gateValuation(valuation, raw, statementEvidence = null) {
  ...
  return { ...valuation, enterpriseValue: semantics.compatible.enterpriseValue ? valuation.enterpriseValue : null, evToEbitda: semantics.compatible.evToEbitda ? valuation.evToEbitda : null, evToRevenue: semantics.compatible.evToRevenue ? valuation.evToRevenue : null, providerMonetarySemantics: semantics };
}
...
export async function analyze(input) {
  ...
  const semantics = {
    source: 'Yahoo Finance quoteSummary.defaultKeyStatistics + quoteSummary.price + fundamentalsTimeSeries',
    tradingCurrency, financialCurrency, currencyStatus, evCurrencyBasis: financialCurrency, observationAt,
    ebitda: { source: 'Yahoo Finance fundamentalsTimeSeries', currency: financialCurrency, period: null, periodType: null },
    revenue: { source: 'Yahoo Finance fundamentalsTimeSeries', currency: financialCurrency, period: null, periodType: null },
  ...
  const gatedValuation = { ...valuation, enterpriseValue: evCompatible ? valuation.enterpriseValue : null, evToEbitda: null, evToRevenue: null, providerMonetarySemantics: semantics };
  const score = core.scoreStock(response.fundamentals, gatedValuation, response.technical, response.dataQuality, response.sectorFramework);
...
export function buildValuation(raw, price, statementEvidence = null) { return gateValuation(core.buildValuation(raw, price), raw, statementEvidence); }
