
import YahooFinance from 'yahoo-finance2';
import { METRIC_DEFINITIONS } from './metric-registry.js';
import { fetchStatementEvidence, mergeStatementEvidence, qualifyAnnualRows } from './statement-evidence.js';
import { technicalCompatibility } from './technical-compatibility-adapter.js';

const yahooFinance = new YahooFinance();

const clamp = (v, a = 0, b = 100) =>
  Math.max(a, Math.min(b, Number.isFinite(v) ? v : a));

const num = (v) => {
  if (typeof v === 'number' && Number.isFinite(v)) return v;
  if (v && typeof v === 'object' && typeof v.raw === 'number' && Number.isFinite(v.raw)) return v.raw;
  return null;
};

const pct = (v) => {
  const n = num(v);
  return n == null ? null : n * 100;
};

function normalizeDebtToEquity(v) {
  const n = num(v);
  if (n == null || n < 0) return null;
  // yahoo-finance2 financialData.debtToEquity is a percentage (e.g. 79.548
  // represents 0.79548x). Statement-derived D/E is handled separately.
  return n / 100;
}

function crore(v) {
  const n = num(v);
  return n == null ? null : n / 1e7;
}

const ticker = (input) => {
  let x = String(input || '').trim().toUpperCase().replace(/\s+/g, '');
  if (!x) throw Error('Stock symbol is required');
  if (x.endsWith('.NS') || x.endsWith('.BO') || x.startsWith('^')) return x;
  return `${x}.NS`;
};

function value(obj, key) {
  return num(obj?.[key]);
}

function normalizeTimeSeriesRows(rows, requestedPeriod) {
  return (Array.isArray(rows) ? rows : [])
    .map((row) => {
      if (!row || typeof row !== 'object') return null;
      const date = row.date instanceof Date ? row.date : new Date(row.date);
      if (Number.isNaN(date.getTime())) return null;
      const periodType = row.periodType == null ? null : String(row.periodType).toUpperCase();
      const providerType = row.TYPE == null ? null : String(row.TYPE).toUpperCase();
      const periodStatus = ['TTM','12M','3M'].includes(periodType) ? 'verified' : 'missing';
      return { ...row, date: date.toISOString(), periodType, providerType, requestedPeriod, periodStatus };
    })
    .filter(Boolean)
    .sort((a,b)=>new Date(a.date)-new Date(b.date));
}

function metricRows(rows, key, {preferTTM=true} = {}) {
  const valid=(rows||[]).filter(r=>num(r?.[key])!=null);
  if(!valid.length) return null;
  if(preferTTM){
    const ttm=valid.filter(r=>r.periodType==='TTM');
    if(ttm.length) return num(ttm.at(-1)[key]);
  }
  return num(valid.at(-1)[key]);
}

function annualRows(rows) {
  return qualifyAnnualRows(rows);
}

function latest(rows, key) {
  const a = (rows || []).filter((r) => num(r?.[key]) != null);
  return a.length ? num(a.at(-1)[key]) : null;
}

function cagr(rows, key, years = 5) {
  const valid = (rows || [])
    .filter((r) => num(r?.[key]) != null)
    .sort((a, b) => new Date(a.date) - new Date(b.date));

  if (valid.length < 2) return null;

  const end = valid.at(-1);
  const targetMs = years * 365.25 * 24 * 60 * 60 * 1000;
  let start = valid[0];

  for (const row of valid) {
    if (new Date(end.date) - new Date(row.date) >= targetMs * 0.75) {
      start = row;
      break;
    }
  }

  const startValue = num(start[key]);
  const endValue = num(end[key]);
  const actualYears = (new Date(end.date) - new Date(start.date)) / (365.25 * 24 * 60 * 60 * 1000);

  if (!(startValue > 0) || !(endValue > 0) || actualYears < 1.5) return null;
  return (Math.pow(endValue / startValue, 1 / actualYears) - 1) * 100;
}

function growthPct(current, previous) {
  if (!(Number.isFinite(current) && Number.isFinite(previous)) || previous === 0) return null;
  return ((current / previous) - 1) * 100;
}

function sma(a, n) {
  return a.length < n ? null : a.slice(-n).reduce((s, v) => s + v, 0) / n;
}

function ema(a, n) {
  if (a.length < n) return null;
  let e = a.slice(0, n).reduce((s, v) => s + v, 0) / n;
  const k = 2 / (n + 1);
  for (let i = n; i < a.length; i++) e = a[i] * k + e * (1 - k);
  return e;
}

function rsi(a, n = 14) {
  if (a.length <= n) return null;
  let gain = 0;
  let loss = 0;
  for (let i = 1; i <= n; i++) {
    const d = a[i] - a[i - 1];
    gain += Math.max(d, 0);
    loss += Math.max(-d, 0);
  }
  gain /= n;
  loss /= n;
  for (let i = n + 1; i < a.length; i++) {
    const d = a[i] - a[i - 1];
    gain = (gain * (n - 1) + Math.max(d, 0)) / n;
    loss = (loss * (n - 1) + Math.max(-d, 0)) / n;
  }
  return loss === 0 ? 100 : 100 - 100 / (1 + gain / loss);
}

function atr(rows, n = 14) {
  // Wilder's ATR: seed with the first n true ranges, then apply
  // Wilder smoothing. This is the standard ATR methodology used by
  // most professional charting implementations.
  if (rows.length <= n) return null;
  const tr = [];
  for (let i = 1; i < rows.length; i++) {
    const x = rows[i];
    const p = rows[i - 1];
    tr.push(Math.max(x.high - x.low, Math.abs(x.high - p.close), Math.abs(x.low - p.close)));
  }
  if (tr.length < n) return null;

  let value = tr.slice(0, n).reduce((sum, x) => sum + x, 0) / n;
  for (let i = n; i < tr.length; i++) {
    value = ((value * (n - 1)) + tr[i]) / n;
  }
  return value;
}

function percentChange(a, n) {
  return a.length <= n ? null : ((a.at(-1) / a.at(-1 - n)) - 1) * 100;
}

function technical(rows) {
  return technicalCompatibility(rows, {
    source: 'Yahoo Finance chart',
    timeframe: '1d',
  });
}

async function fetchChart(symbol) {
  const data = await yahooFinance.chart(symbol, {
    period1: new Date(Date.now() - 2 * 365.25 * 24 * 60 * 60 * 1000),
    period2: new Date(),
    interval: '1d',
    events: 'div,splits',
    return: 'object',
  });

  const rows = [];
  const retrievedAt = new Date().toISOString();

  // yahoo-finance2 v3 returns the array format by default. We explicitly request
  // the object format above, but keep this parser compatible with both formats.
  if (Array.isArray(data?.quotes)) {
    for (const item of data.quotes) {
      const close = num(item?.close);
      const high = num(item?.high);
      const low = num(item?.low);
      const volume = num(item?.volume);
      if (close != null && high != null && low != null && volume != null) {
        rows.push({
          date: new Date(item.date).toISOString(),
          open: num(item?.open),
          close, high, low, volume,
          retrievedAt,
        });
      }
    }
  } else {
    const timestamps = data?.timestamp || [];
    const q = data?.indicators?.quote?.[0] || {};
    for (let i = 0; i < timestamps.length; i++) {
      const close = num(q.close?.[i]);
      const high = num(q.high?.[i]);
      const low = num(q.low?.[i]);
      const volume = num(q.volume?.[i]);
      if (close != null && high != null && low != null && volume != null) {
        rows.push({
          date: new Date(timestamps[i] * 1000).toISOString(),
          open: num(q.open?.[i]),
          close, high, low, volume,
          retrievedAt,
        });
      }
    }
  }

  if (rows.length < 30) throw Error(`Insufficient market data (${rows.length} points)`);

  return {
    symbol,
    currency: data?.meta?.currency || 'INR',
    exchange: data?.meta?.exchangeName || '',
    price: num(data?.meta?.regularMarketPrice) ?? rows.at(-1).close,
    rows,
  };
}

async function fetchFundamentals(symbol) {
  const period1 = new Date(Date.now() - 7 * 365.25 * 24 * 60 * 60 * 1000);
  const period2 = new Date();

  const [summaryResult, annualResult, trailingResult] = await Promise.allSettled([
    yahooFinance.quoteSummary(symbol, {
      modules: ['price','summaryDetail','defaultKeyStatistics','financialData','assetProfile','majorHoldersBreakdown','institutionOwnership','insiderHolders','insiderTransactions','earningsTrend','quoteType'],
    }, { validateResult: false }),
    yahooFinance.fundamentalsTimeSeries(symbol, { period1, period2, type: 'annual', module: 'all' }, { validateResult: false }),
    yahooFinance.fundamentalsTimeSeries(symbol, { period1: new Date(Date.now() - 24 * 30 * 24 * 60 * 60 * 1000), period2, type: 'trailing', module: 'all' }, { validateResult: false }),
  ]);

  const summary = summaryResult.status === 'fulfilled' ? summaryResult.value : null;
  const annual = annualResult.status === 'fulfilled' ? normalizeTimeSeriesRows(annualResult.value, 'annual') : [];
  const trailing = trailingResult.status === 'fulfilled' ? normalizeTimeSeriesRows(trailingResult.value, 'trailing') : [];

  return {
    summary, annual, trailing,
    fetchedAt: new Date().toISOString(),
    errors: {
      summary: summaryResult.status === 'rejected' ? String(summaryResult.reason?.message || summaryResult.reason) : null,
      annual: annualResult.status === 'rejected' ? String(annualResult.reason?.message || annualResult.reason) : null,
      trailing: trailingResult.status === 'rejected' ? String(trailingResult.reason?.message || trailingResult.reason) : null,
    },
    providerValidation: {
      summary: false, annual: false, trailing: false,
      reason: 'Yahoo Finance payloads are consumed with provider validation disabled and are validated by the StockSamjho normalization layer. TTM/ALL are accepted when structurally usable.',
    },
  };
}

function buildFinancials(raw) {
  const f = raw.summary?.financialData || {};
  const k = raw.summary?.defaultKeyStatistics || {};
  const d = raw.summary?.summaryDetail || {};

  // fundamentalsTimeSeries(module:'all') can return TYPE:'ALL' rows.
  // Treat those as statement rows too, otherwise valid financial history is discarded.
  const allFinancialRows = [...(raw.annual || []), ...(raw.trailing || [])];
  const annualFinancials = annualRows(raw.annual).filter((r) => r?.providerType === 'FINANCIALS' || r?.providerType === 'ALL' || r?.TYPE === 'FINANCIALS' || r?.TYPE === 'ALL');
  const balanceRows = annualRows(raw.annual).filter((r) => r?.providerType === 'BALANCE_SHEET' || r?.TYPE === 'BALANCE_SHEET');
  const cashRows = annualRows(raw.annual).filter((r) => r?.providerType === 'CASH_FLOW' || r?.TYPE === 'CASH_FLOW');
  const usableFinancialRows = [...annualFinancials, ...balanceRows, ...cashRows];
  const annualBalance = balanceRows;
  const annualCash = cashRows;

  const revenue = metricRows(raw.trailing, 'totalRevenue') ?? metricRows(usableFinancialRows, 'totalRevenue') ?? value(f, 'totalRevenue');
  const ebitda = metricRows(raw.trailing, 'EBITDA') ?? metricRows(usableFinancialRows, 'EBITDA') ?? value(f, 'ebitda');
  const netIncome = metricRows(raw.trailing, 'netIncomeFromContinuingAndDiscontinuedOperation') ?? metricRows(usableFinancialRows, 'netIncomeFromContinuingAndDiscontinuedOperation') ?? value(k, 'netIncomeToCommon');
  const freeCashFlow = value(f, 'freeCashflow') ?? metricRows(raw.trailing, 'freeCashFlow') ?? metricRows(usableFinancialRows, 'freeCashFlow');
  const operatingCashFlow = value(f, 'operatingCashflow') ?? metricRows(raw.trailing, 'operatingCashFlow') ?? metricRows(usableFinancialRows, 'operatingCashFlow');

  const totalDebt = value(f, 'totalDebt') ?? metricRows(balanceRows, 'totalDebt');
  const cash = value(f, 'totalCash') ?? metricRows(balanceRows, 'cashCashEquivalentsAndShortTermInvestments');
  const netDebt = metricRows(usableFinancialRows, 'netDebt') ?? (totalDebt != null && cash != null ? totalDebt - cash : null);
  const equity = metricRows(balanceRows, 'stockholdersEquity') ?? metricRows(balanceRows, 'commonStockEquity');

  const shares = value(k, 'sharesOutstanding') ?? metricRows(usableFinancialRows, 'ordinarySharesNumber');

  const revenue3y = cagr(annualFinancials, 'totalRevenue', 3);
  const revenue5y = cagr(annualFinancials, 'totalRevenue', 5);
  const eps3y = cagr(annualFinancials, 'dilutedEPS', 3);
  const eps5y = cagr(annualFinancials, 'dilutedEPS', 5);
  const pat3y = cagr(annualFinancials, 'netIncomeFromContinuingAndDiscontinuedOperation', 3);
  const pat5y = cagr(annualFinancials, 'netIncomeFromContinuingAndDiscontinuedOperation', 5);

  const currentAssets = latest(annualBalance, 'currentAssets');
  const currentLiabilities = latest(annualBalance, 'currentLiabilities');

  const revenueCurrent = latest(annualFinancials, 'totalRevenue');
  const revenuePrev = annualFinancials.length >= 2 ? num(annualFinancials.at(-2)?.totalRevenue) : null;
  const epsCurrent = latest(annualFinancials, 'dilutedEPS');
  const epsPrev = annualFinancials.length >= 2 ? num(annualFinancials.at(-2)?.dilutedEPS) : null;
  const fcfCurrent = latest(annualFinancials, 'freeCashFlow');
  const fcfPrev = annualFinancials.length >= 2 ? num(annualFinancials.at(-2)?.freeCashFlow) : null;
  const interestExpense = value(f, 'interestExpense') ?? metricRows(usableFinancialRows, 'interestExpense');
  const operatingIncome = value(f, 'operatingIncome') ?? metricRows(raw.trailing, 'operatingIncome') ?? metricRows(usableFinancialRows, 'operatingIncome');
  const totalAssets = latest(annualBalance, 'totalAssets');

  return {
    current: {
      revenue,
      ebitda,
      netIncome,
      freeCashFlow,
      operatingCashFlow,
      totalDebt,
      netDebt,
      cash,
      equity,
      shares,
      currentAssets,
      currentLiabilities,
    },
    ratios: {
      roe: pct(value(f, 'returnOnEquity')),
      roa: pct(value(f, 'returnOnAssets')),
      debtToEquity: normalizeDebtToEquity(value(f, 'debtToEquity')),
      currentRatio: value(f, 'currentRatio'),
      quickRatio: value(f, 'quickRatio'),
      grossMargin: pct(value(f, 'grossMargins')),
      operatingMargin: pct(value(f, 'operatingMargins')),
      netMargin: pct(value(f, 'profitMargins')),
      ebitdaMargin: pct(value(f, 'ebitdaMargins')),
      revenueGrowth: pct(value(f, 'revenueGrowth')),
      earningsGrowth: pct(value(f, 'earningsGrowth')),
      earningsQuarterlyGrowth: pct(value(k, 'earningsQuarterlyGrowth')),
    },
    growth: {
      revenue3yCagr: revenue3y,
      revenue5yCagr: revenue5y,
      eps3yCagr: eps3y,
      eps5yCagr: eps5y,
      pat3yCagr: pat3y,
      pat5yCagr: pat5y,
      latestRevenueGrowth: growthPct(revenueCurrent, revenuePrev),
      latestEPSGrowth: growthPct(epsCurrent, epsPrev),
    },
    derived: {
      debtToEquityFromStatements:
        equity && totalDebt != null ? totalDebt / equity : null,
      netDebtToEbitda:
        netDebt != null && ebitda > 0 ? netDebt / ebitda : null,
      fcfMargin:
        freeCashFlow != null && revenue > 0 ? (freeCashFlow / revenue) * 100 : null,
      fcfConversion:
        freeCashFlow != null && netIncome > 0 ? (freeCashFlow / netIncome) * 100 : null,
      currentRatioFromStatements:
        currentAssets != null && currentLiabilities > 0 ? currentAssets / currentLiabilities : null,
      interestCoverage:
        operatingIncome != null && interestExpense != null && interestExpense > 0
          ? operatingIncome / interestExpense : null,
      roeFromStatements:
        netIncome != null && equity != null && equity > 0 ? (netIncome / equity) * 100 : null,
      roaFromStatements:
        netIncome != null && totalAssets != null && totalAssets > 0 ? (netIncome / totalAssets) * 100 : null,
      roceFromStatements:
        operatingIncome != null && equity != null && totalDebt != null && (equity + totalDebt) > 0
          ? (operatingIncome / (equity + totalDebt)) * 100 : null,
      fcfGrowth: growthPct(fcfCurrent, fcfPrev),
      workingCapital: currentAssets != null && currentLiabilities != null
        ? currentAssets - currentLiabilities : null,
    },
    sectorKpis: { nim: null, gnpa: null, nnpa: null, casa: null, creditGrowth: null, provisionCoverage: null, attrition: null, utilization: null, dealWins: null, clientConcentration: null, capex: null, capacityUtilization: null, volumeGrowth: null, asp: null, marketShare: null },
    sourceNote: 'Yahoo Finance quoteSummary + fundamentalsTimeSeries. Missing fields remain null; no financial metric is fabricated.',
    periods: { currentMetrics: raw.trailing.some(r => r.periodType === 'TTM') ? 'TTM' : (raw.trailing.at(-1)?.periodType || null), annualHistoryCount: annualFinancials.length, historicalGrowth: '12M/annual only; TTM excluded from CAGR baselines' },
    rawAvailability: {
      quoteSummary: Boolean(raw.summary),
      annualStatements: annualFinancials.length > 0,
      annualBalanceSheet: annualBalance.some(r => ['totalDebt','stockholdersEquity','commonStockEquity','currentAssets','currentLiabilities','cashCashEquivalentsAndShortTermInvestments'].some(k => num(r?.[k]) != null)),
      annualCashFlow: annualCash.some(r => ['freeCashFlow','operatingCashFlow','cashFlowFromContinuingOperatingActivities'].some(k => num(r?.[k]) != null)),
    },
  };
}

function buildValuation(raw, price) {
  const d = raw.summary?.summaryDetail || {};
  const k = raw.summary?.defaultKeyStatistics || {};
  const f = raw.summary?.financialData || {};

  const allFinancialRows = [...(raw.annual || []), ...(raw.trailing || [])];
  const usableFinancialRows = allFinancialRows.filter((r) =>
    r?.providerType === 'FINANCIALS' || r?.providerType === 'ALL' ||
    r?.TYPE === 'FINANCIALS' || r?.TYPE === 'ALL'
  );
  const sharesOutstanding =
    value(k, 'sharesOutstanding') ??
    metricRows(usableFinancialRows, 'ordinarySharesNumber') ??
    metricRows(usableFinancialRows, 'sharesOutstanding');
  const reportedMarketCap = value(d, 'marketCap') ?? value(raw.summary?.price, 'marketCap');
  const calculatedMarketCap =
    sharesOutstanding != null && price != null && sharesOutstanding > 0 && price > 0
      ? sharesOutstanding * price : null;
  const marketCap = reportedMarketCap != null && reportedMarketCap > 0
    ? reportedMarketCap : calculatedMarketCap;
  const marketCapStatus =
    reportedMarketCap != null && reportedMarketCap > 0 ? 'verified' :
    calculatedMarketCap != null ? 'calculated' : 'missing';
  const trailingPE = value(d, 'trailingPE');
  const forwardPE = value(d, 'forwardPE') ?? value(k, 'forwardPE');
  const priceToBook = value(k, 'priceToBook');
  const pegRatio = value(k, 'pegRatio');
  const enterpriseValue = value(k, 'enterpriseValue');
  const evToEbitda = value(k, 'enterpriseToEbitda');
  const evToRevenue = value(k, 'enterpriseToRevenue');
  const trailingEps = value(k, 'trailingEps');
  const forwardEps = value(k, 'forwardEps');
  const bookValue = value(k, 'bookValue');

  // Build a growth signal from multiple independent sources.
  const annualFinancials = annualRows(raw.annual);
  const eps3y = cagr(annualFinancials, 'dilutedEPS', 3);
  const eps5y = cagr(annualFinancials, 'dilutedEPS', 5);
  const pat3y = cagr(annualFinancials, 'netIncomeFromContinuingAndDiscontinuedOperation', 3);
  const pat5y = cagr(annualFinancials, 'netIncomeFromContinuingAndDiscontinuedOperation', 5);
  const rev3y = cagr(annualFinancials, 'totalRevenue', 3);
  const rev5y = cagr(annualFinancials, 'totalRevenue', 5);
  const earningsGrowth = pct(value(f, 'earningsGrowth'));
  const revenueGrowth = pct(value(f, 'revenueGrowth'));
  const roe = pct(value(f, 'returnOnEquity'));

  const growthCandidates = [eps3y, eps5y, pat3y, pat5y, earningsGrowth]
    .filter((x) => Number.isFinite(x) && x > -50 && x < 100);
  const growthSignal = growthCandidates.length
    ? growthCandidates.reduce((a, b) => a + b, 0) / growthCandidates.length
    : null;

  // A justified multiple is deliberately more conservative than simply using
  // the market's current P/E. This prevents fair value = current price.
  const qualityPremium =
    roe == null ? 0 :
    roe >= 30 ? 4 :
    roe >= 20 ? 2.5 :
    roe >= 15 ? 1 :
    0;

  const baseGrowth = growthSignal == null ? null : Math.max(-5, Math.min(30, growthSignal));
  const conservativeGrowth = baseGrowth == null ? null : Math.max(-5, baseGrowth * 0.70);
  const optimisticGrowth = baseGrowth == null ? null : Math.min(35, Math.max(-5, baseGrowth * 1.20));

  const justifiedPE = (growth, premium = qualityPremium) =>
    growth == null ? null : Math.max(10, Math.min(32, 12 + growth * 0.75 + premium));

  const conservativePE = justifiedPE(conservativeGrowth);
  const basePE = justifiedPE(baseGrowth);
  const optimisticPE = justifiedPE(optimisticGrowth);

  const forwardFair = (eps, pe) => eps > 0 && pe > 0 ? eps * pe : null;
  const trailingFair = (eps, pe) => eps > 0 && pe > 0 ? eps * pe : null;

  const conservativeValues = [
    forwardFair(forwardEps, conservativePE),
    trailingFair(trailingEps, conservativePE)
  ].filter(Number.isFinite);

  const baseValues = [
    forwardFair(forwardEps, basePE),
    trailingFair(trailingEps, basePE)
  ].filter(Number.isFinite);

  const optimisticValues = [
    forwardFair(forwardEps, optimisticPE),
    trailingFair(trailingEps, optimisticPE)
  ].filter(Number.isFinite);

  const avg = (a) => a.length ? a.reduce((x, y) => x + y, 0) / a.length : null;
  const conservativeFairValue = avg(conservativeValues);
  const baseFairValue = avg(baseValues);
  const optimisticFairValue = avg(optimisticValues);

  // If EPS is unavailable, do not manufacture an earnings fair value.
  // P/B is retained as a reference for financial companies, not mixed blindly
  // into the primary fair-value estimate.
  const pbReference = bookValue > 0 && priceToBook > 0 ? bookValue * priceToBook : null;
  const primaryFairValue = baseFairValue;

  const upside = (fv) => fv && price ? ((fv / price) - 1) * 100 : null;
  const mos = (fv) => fv && price ? (1 - price / fv) * 100 : null;

  const valuationGap = primaryFairValue && price ? ((price / primaryFairValue) - 1) * 100 : null;
  const verdict =
    primaryFairValue == null ? 'DATA INSUFFICIENT' :
    valuationGap <= -35 ? 'DEEPLY UNDERVALUED' :
    valuationGap <= -20 ? 'UNDERVALUED' :
    valuationGap <= -8 ? 'ATTRACTIVE' :
    valuationGap <= 8 ? 'FAIR / REASONABLE' :
    valuationGap <= 20 ? 'EXPENSIVE' :
    'VERY EXPENSIVE';

  return {
    currentPrice: price,
    marketCap,
    marketCapStatus,
    sharesOutstanding,
    trailingPE,
    forwardPE,
    priceToBook,
    pegRatio,
    enterpriseValue,
    evToEbitda,
    evToRevenue,
    trailingEPS: trailingEps,
    forwardEPS: forwardEps,
    bookValue,
    earningsYield: trailingPE > 0 ? (1 / trailingPE) * 100 : null,

    growthSignal,
    revenueGrowth,
    revenue3yCagr: rev3y,
    revenue5yCagr: rev5y,
    eps3yCagr: eps3y,
    eps5yCagr: eps5y,
    pat3yCagr: pat3y,
    pat5yCagr: pat5y,
    roe,

    conservativeGrowth,
    baseGrowth,
    optimisticGrowth,
    conservativePE,
    basePE,
    optimisticPE,
    conservativeFairValue,
    baseFairValue,
    optimisticFairValue,
    fairValue: primaryFairValue,
    pbReference,
    upsideToFairValue: upside(primaryFairValue),
    marginOfSafety: mos(primaryFairValue),
    valuationGap,
    verdict,
    method: 'Growth-adjusted justified P/E using EPS history, earnings growth and ROE; P/B shown separately as a reference.',
    warning:
      primaryFairValue == null
        ? 'No defensible earnings-based fair value available from the returned data.'
        : 'Scenario fair values are framework estimates, not analyst targets or guarantees.',
    verdictThresholds: {
      deeplyUndervaluedAt: '<= -35% valuation gap',
      undervaluedAt: '-35% to -20%',
      attractiveAt: '-20% to -8%',
      fairAt: '-8% to +8%',
      expensiveAt: '+8% to +20%',
      veryExpensiveAt: '> +20%',
    },
  };
}
function buildOwnership(raw) {
  const m = raw.summary?.majorHoldersBreakdown || {};
  const k = raw.summary?.defaultKeyStatistics || {};

  return {
    insidersPct: pct(value(k, 'heldPercentInsiders')),
    institutionsPct: pct(value(k, 'heldPercentInstitutions')),
    majorHolders: m,
    institutionOwnership: raw.summary?.institutionOwnership?.ownershipList || [],
    insiderHolders: raw.summary?.insiderHolders?.holders || [],
    insiderTransactions: raw.summary?.insiderTransactions?.transactions || [],
    sourceNote: 'Ownership fields depend on Yahoo availability and may be delayed or incomplete.',
  };
}

function freshnessScore(asOf, now = Date.now()) {
  if (!asOf) return 0;
  const ts = new Date(asOf).getTime();
  if (!Number.isFinite(ts)) return 0;
  const ageHours = Math.max(0, (now - ts) / 36e5);
  if (ageHours <= 24) return 100;
  if (ageHours <= 72) return 90;
  if (ageHours <= 168) return 75;
  if (ageHours <= 720) return 50;
  return 25;
}

function buildDataQuality(financials, valuation, ownership, technical, freshness = {}, sectorFramework = null) {
  const checks = [
    ['price', valuation.currentPrice, 1.0, true],
    ['marketCap', valuation.marketCap, 0.75, true],
    ['P/E', valuation.trailingPE, 1.0, true],
    ['Forward P/E', valuation.forwardPE, 0.5, false],
    ['P/B', valuation.priceToBook, 0.5, false],
    ['Fair Value', valuation.fairValue, 1.0, true],
    ['Growth Signal', valuation.growthSignal, 0.75, true],
    ['ROE', financials.ratios.roe, 1.0, true],
    ['ROA', financials.ratios.roa, 0.5, false],
    ['Debt/Equity', financials.ratios.debtToEquity, 1.0, true],
    ['Net Debt', financials.current.netDebt, 0.75, true],
    ['Net Margin', financials.ratios.netMargin, 1.0, true],
    ['Operating Margin', financials.ratios.operatingMargin, 0.5, false],
    ['Revenue Growth', financials.ratios.revenueGrowth, 1.0, true],
    ['Earnings Growth', financials.ratios.earningsGrowth, 1.0, true],
    ['Free Cash Flow', financials.current.freeCashFlow, 1.0, true],
    ['FCF Conversion', financials.derived.fcfConversion, 0.5, false],
    ['5Y Revenue CAGR', financials.growth.revenue5yCagr, 1.0, true],
    ['5Y EPS CAGR', financials.growth.eps5yCagr, 1.0, true],
    ['RSI', technical.rsi, 0.5, false],
    ['200 DMA', technical.s200, 1.0, true],
    ['EV/EBITDA', valuation.evToEbitda, 0.5, false],
    ['52W Range', technical.high52Week != null && technical.low52Week != null ? 1 : null, 0.5, false],
    ['Drawdown', technical.drawdown, 0.5, false],
    ['Volume Trend', technical.volumeTrend, 0.25, false],
  ];

  const totalWeight = checks.reduce((sum, [, value, weight]) => sum + weight, 0);
  const availableWeight = checks.filter(([, value]) => value != null).reduce((sum, [, , weight]) => sum + weight, 0);
  const rawCoverage = totalWeight ? (availableWeight / totalWeight) * 100 : 0;
  const missing = checks.filter(([, value]) => value == null).map(([name]) => name);
  const criticalMissing = checks.filter(([, value, , critical]) => critical && value == null).map(([name]) => name);

  const validationFlags = [];
  if (financials.ratios.debtToEquity != null && financials.ratios.debtToEquity > 10) validationFlags.push('Debt/Equity is unusually high; verify statement data.');
  if (financials.ratios.currentRatio != null && financials.ratios.currentRatio < 0) validationFlags.push('Current ratio is negative; verify provider data.');
  if (financials.ratios.netMargin != null && (financials.ratios.netMargin < -100 || financials.ratios.netMargin > 100)) validationFlags.push('Net margin is outside the expected range.');
  if (financials.derived.fcfConversion != null && Math.abs(financials.derived.fcfConversion) > 500) validationFlags.push('FCF conversion is unusually large; inspect working-capital/capex effects.');

  const providerErrors = Object.values(financials.errors || {}).filter(Boolean);
  const warnings = [
    ...(!financials.rawAvailability.quoteSummary ? ['Quote summary unavailable.'] : []),
    ...(!financials.rawAvailability.annualBalanceSheet ? ['Annual balance-sheet coverage is unavailable; Financial Strength is capped.'] : []),
    ...(!financials.rawAvailability.annualCashFlow ? ['Annual cash-flow coverage is unavailable; cash-flow evidence is incomplete.'] : []),
    ...(!financials.rawAvailability.annualStatements ? ['Annual financial statements unavailable.'] : []),
    ...(criticalMissing.length ? [`Critical fields unavailable: ${criticalMissing.join(', ')}.`] : []),
    ...validationFlags,
    ...providerErrors.map((x) => `Provider error: ${x}`),
  ];

  const annualHistoryRows = financials.periods?.annualHistoryCount ?? 0;
  const historyScore = annualHistoryRows >= 5 ? 100 : annualHistoryRows === 4 ? 90 : annualHistoryRows === 3 ? 75 : annualHistoryRows === 2 ? 55 : annualHistoryRows === 1 ? 35 : 0;
  const marketFreshness = freshnessScore(freshness.marketAsOf);
  const fundamentalsFreshness = freshnessScore(freshness.fundamentalsAsOf);
  const freshnessValues = [marketFreshness, fundamentalsFreshness].filter((x) => x > 0);
  const freshnessComponent = freshnessValues.length ? freshnessValues.reduce((a, b) => a + b, 0) / freshnessValues.length : 0;
  const consistencyComponent = Math.max(0, 100 - validationFlags.length * 15 - providerErrors.length * 20);

  const confidenceComponents = {
    coverage: Math.round(rawCoverage),
    freshness: Math.round(freshnessComponent),
    consistency: Math.round(consistencyComponent),
    historicalDepth: historyScore,
  };
  const confidenceWeights = { coverage: 0.45, freshness: 0.20, consistency: 0.20, historicalDepth: 0.15 };
  let confidence = Math.round(
    confidenceComponents.coverage * confidenceWeights.coverage +
    confidenceComponents.freshness * confidenceWeights.freshness +
    confidenceComponents.consistency * confidenceWeights.consistency +
    confidenceComponents.historicalDepth * confidenceWeights.historicalDepth
  );
  const capsApplied = [];
  if (!financials.rawAvailability.annualStatements) { confidence = Math.min(confidence, 70); capsApplied.push('annualStatements unavailable'); }
  if (!financials.rawAvailability.annualBalanceSheet) { confidence = Math.min(confidence, 75); capsApplied.push('annualBalanceSheet unavailable'); }
  if (!financials.rawAvailability.annualCashFlow) { confidence = Math.min(confidence, 80); capsApplied.push('annualCashFlow unavailable'); }
  if (criticalMissing.length >= 3) { confidence = Math.min(confidence, 65); capsApplied.push('3+ critical fields unavailable'); }
  else if (criticalMissing.length === 2) { confidence = Math.min(confidence, 75); capsApplied.push('2 critical fields unavailable'); }
  else if (criticalMissing.length === 1) { confidence = Math.min(confidence, 85); capsApplied.push('1 critical field unavailable'); }
  if (sectorFramework?.coverage != null && sectorFramework.coverage < 50) { confidence = Math.min(confidence, 65); capsApplied.push(`sector KPI coverage ${sectorFramework.coverage}%`); }
  confidence = Math.max(0, Math.min(100, confidence));

  const completeness = Math.round(rawCoverage);
  const verifiedFields = [
    'price','marketCap','P/E','Forward P/E','P/B','ROE','ROA','Debt/Equity',
    'Net Debt','Net Margin','Operating Margin','Revenue Growth','Earnings Growth',
    'Free Cash Flow','RSI','200 DMA','EV/EBITDA'
  ].filter(name => !missing.includes(name));
  const calculatedFields = [
    '5Y Revenue CAGR','5Y EPS CAGR','Growth Signal','Fair Value','FCF Conversion'
  ].filter(name => !missing.includes(name));
  if (valuation.marketCapStatus === 'calculated') {
    const i = verifiedFields.indexOf('marketCap');
    if (i >= 0) verifiedFields.splice(i, 1);
    calculatedFields.push('marketCap');
  }

  return {
    completeness,
    confidence,
    confidenceModel: {
      id: 'data-confidence-v1.0-heuristic',
      status: 'INITIAL MODEL — SUBJECT TO BACKTESTING',
      formula: '45% coverage + 20% freshness + 20% consistency + 15% historical depth, then explicit evidence caps',
      weights: confidenceWeights,
      components: confidenceComponents,
      capsApplied,
    },
    available: checks.filter(([, value]) => value != null).length,
    total: checks.length,
    providerFields: verifiedFields,
    calculatedFields,
    missingFields: missing,
    criticalMissingFields: criticalMissing,
    staleFields: [],
    status: completeness >= 85 ? 'STRONG' : completeness >= 65 ? 'PARTIAL' : 'WEAK',
    warnings,
    validationFlags,
    sectorFramework: sectorFramework || null,
    financialStrengthCoverage: Boolean(financials.rawAvailability.annualBalanceSheet) && Boolean(financials.rawAvailability.annualCashFlow)
      ? 'FULL'
      : Boolean(financials.rawAvailability.annualBalanceSheet) || Boolean(financials.rawAvailability.annualCashFlow)
        ? 'PARTIAL'
        : 'CURRENT_FIELDS_ONLY',
  };
}

const SECTOR_FRAMEWORKS = {
  BANKING: {
    id: 'banking-v1.0', label: 'Banking & Financial Services',
    required: ['ROE', 'ROA', 'NIM', 'GNPA', 'NNPA', 'CASA', 'Credit Growth', 'Provision Coverage'],
    availableMetricKeys: { 'ROE': 'roe', 'ROA': 'roa', 'NIM': 'nim', 'GNPA': 'gnpa', 'NNPA': 'nnpa', 'CASA': 'casa', 'Credit Growth': 'creditGrowth', 'Provision Coverage': 'provisionCoverage' },
    unavailablePolicy: 'Do not substitute corporate Debt/Equity or Current Ratio for banking asset-quality KPIs.'
  },
  IT_SERVICES: {
    id: 'it-services-v1.0', label: 'IT Services',
    required: ['Revenue Growth', 'Operating Margin', 'EPS Growth', 'Attrition', 'Utilization', 'Deal Wins', 'Client Concentration'],
    availableMetricKeys: { 'Revenue Growth': 'revenueGrowth', 'Operating Margin': 'operatingMargin', 'EPS Growth': 'epsGrowth', 'Attrition': 'attrition', 'Utilization': 'utilization', 'Deal Wins': 'dealWins', 'Client Concentration': 'clientConcentration' },
    unavailablePolicy: 'Operational KPIs are only used when sourced; they are never inferred from price or margins.'
  },
  MANUFACTURING: {
    id: 'manufacturing-v1.0', label: 'Manufacturing',
    required: ['ROCE', 'Operating Margin', 'Revenue Growth', 'Working Capital', 'Capex', 'Capacity Utilization'],
    availableMetricKeys: { 'ROCE': 'roce', 'Operating Margin': 'operatingMargin', 'Revenue Growth': 'revenueGrowth', 'Working Capital': 'workingCapital', 'Capex': 'capex', 'Capacity Utilization': 'capacityUtilization' },
    unavailablePolicy: 'Capacity and capex KPIs require sourced disclosures; they are not estimated from revenue alone.'
  },
  AUTO: {
    id: 'auto-v1.0', label: 'Automotive',
    required: ['Revenue Growth', 'Operating Margin', 'EPS Growth', 'Volume Growth', 'ASP', 'Market Share', 'Capacity Utilization'],
    availableMetricKeys: { 'Revenue Growth': 'revenueGrowth', 'Operating Margin': 'operatingMargin', 'EPS Growth': 'epsGrowth', 'Volume Growth': 'volumeGrowth', 'ASP': 'asp', 'Market Share': 'marketShare', 'Capacity Utilization': 'capacityUtilization' },
    unavailablePolicy: 'Vehicle volumes, ASP and market share require sourced operating disclosures.'
  },
  DEFAULT: {
    id: 'general-v1.0', label: 'General Corporate',
    required: ['ROE', 'ROCE', 'Debt/Equity', 'Revenue Growth', 'FCF Conversion'],
    availableMetricKeys: { 'ROE': 'roe', 'ROCE': 'roce', 'Debt/Equity': 'debtToEquity', 'Revenue Growth': 'revenueGrowth', 'FCF Conversion': 'fcfConversion' },
    unavailablePolicy: 'Only sourced metrics are used; unavailable sector KPIs remain unavailable.'
  },
};
function classifySector(sector, industry) {
  const s = `${sector || ''} ${industry || ''}`.toLowerCase();
  if (/bank|credit|financial services|financial sector/.test(s)) return 'BANKING';
  if (/information technology|software|it services|technology/.test(s)) return 'IT_SERVICES';
  if (/automobile|automotive|auto components|auto manufacturers|vehicle/.test(s)) return 'AUTO';
  if (/industrial|manufactur|machinery|capital goods|chemicals|metals|materials/.test(s)) return 'MANUFACTURING';
  return 'DEFAULT';
}
function buildSectorFramework(financials, sector, industry) {
  const key = classifySector(sector, industry), framework = SECTOR_FRAMEWORKS[key];
  const values = {
    roe: financials?.ratios?.roe ?? null, roa: financials?.ratios?.roa ?? null,
    nim: financials?.sectorKpis?.nim ?? null, gnpa: financials?.sectorKpis?.gnpa ?? null, nnpa: financials?.sectorKpis?.nnpa ?? null,
    casa: financials?.sectorKpis?.casa ?? null, creditGrowth: financials?.sectorKpis?.creditGrowth ?? null, provisionCoverage: financials?.sectorKpis?.provisionCoverage ?? null,
    revenueGrowth: financials?.ratios?.revenueGrowth ?? null, operatingMargin: financials?.ratios?.operatingMargin ?? null, epsGrowth: financials?.ratios?.earningsGrowth ?? null,
    attrition: financials?.sectorKpis?.attrition ?? null, utilization: financials?.sectorKpis?.utilization ?? null, dealWins: financials?.sectorKpis?.dealWins ?? null, clientConcentration: financials?.sectorKpis?.clientConcentration ?? null,
    roce: financials?.derived?.roceFromStatements ?? null, workingCapital: financials?.derived?.workingCapital ?? null, capex: financials?.sectorKpis?.capex ?? null, capacityUtilization: financials?.sectorKpis?.capacityUtilization ?? null,
    volumeGrowth: financials?.sectorKpis?.volumeGrowth ?? null, asp: financials?.sectorKpis?.asp ?? null, marketShare: financials?.sectorKpis?.marketShare ?? null,
    debtToEquity: financials?.ratios?.debtToEquity ?? null, fcfConversion: financials?.derived?.fcfConversion ?? null,
  };
  const rows = framework.required.map(name => ({ name, value: values[framework.availableMetricKeys[name]] ?? null }));
  const available = rows.filter(x => x.value != null).length, coverage = rows.length ? Math.round((available / rows.length) * 100) : 0;
  return { key, id: framework.id, label: framework.label, required: rows, available, total: rows.length, coverage, status: coverage >= 75 ? 'GOOD' : coverage >= 50 ? 'PARTIAL' : 'LIMITED', policy: framework.unavailablePolicy };
}

const SCORE_MODEL = {
  id: 'compounder-v1.0-heuristic',
  status: 'INITIAL MODEL — SUBJECT TO BACKTESTING',
  weights: {
    quality: 0.22,
    growth: 0.15,
    wealth: 0.14,
    financialStrength: 0.18,
    valuation: 0.14,
    technical: 0.05,
    inverseRisk: 0.04,
    confidence: 0.08,
  },
};

function scoreStock(financials, valuation, technical, quality, sectorFramework = null) {
  const evidenceConfidence = Number.isFinite(quality?.confidence) ? quality.confidence : (Number.isFinite(quality?.completeness) ? quality.completeness : 0);
  const r = financials.ratios;
  const g = financials.growth;
  const d = financials.derived;

  let q = 50;
  if (r.roe != null) q += r.roe >= 25 ? 18 : r.roe >= 20 ? 14 : r.roe >= 15 ? 9 : r.roe >= 10 ? 4 : -6;
  if (r.operatingMargin != null) q += r.operatingMargin >= 25 ? 10 : r.operatingMargin >= 15 ? 7 : r.operatingMargin >= 10 ? 3 : -3;
  if (r.netMargin != null) q += r.netMargin >= 20 ? 8 : r.netMargin >= 12 ? 5 : r.netMargin >= 7 ? 2 : -3;
  if (d.fcfConversion != null) q += d.fcfConversion >= 100 ? 8 : d.fcfConversion >= 80 ? 5 : d.fcfConversion >= 60 ? 2 : -5;
  if (g.revenue5yCagr != null) q += g.revenue5yCagr >= 15 ? 8 : g.revenue5yCagr >= 10 ? 5 : g.revenue5yCagr >= 5 ? 2 : 0;
  q = clamp(q);

  const growthRates = [g.revenue5yCagr, g.eps5yCagr, g.pat5yCagr].filter(Number.isFinite);
  let growth = 50;
  if (growthRates.length) {
    const avg = growthRates.reduce((a, b) => a + b, 0) / growthRates.length;
    growth = clamp(50 + avg * 2.2);
  }

  // Financial strength must be evidence-aware. The previous additive baseline
  // could reach an apparently excellent score even when the annual balance
  // sheet was unavailable. Score only the supplied evidence, then apply a
  // transparent coverage cap when statement coverage is incomplete.
  const financialStrengthSignals = [];
  if (sectorFramework?.key === 'BANKING') {
    if (r.roe != null) financialStrengthSignals.push({ value: r.roe >= 18 ? 90 : r.roe >= 14 ? 78 : r.roe >= 10 ? 65 : 45, weight: 0.55, name: 'ROE' });
    if (r.roa != null) financialStrengthSignals.push({ value: r.roa >= 1.5 ? 90 : r.roa >= 1 ? 78 : r.roa >= 0.5 ? 62 : 40, weight: 0.45, name: 'ROA' });
  } else {
    if (r.debtToEquity != null) financialStrengthSignals.push({ value: r.debtToEquity <= 0.25 ? 90 : r.debtToEquity <= 0.75 ? 78 : r.debtToEquity <= 1.5 ? 60 : r.debtToEquity <= 2.5 ? 42 : 25, weight: 0.40, name: 'Debt/Equity' });
    if (d.netDebtToEbitda != null) financialStrengthSignals.push({ value: d.netDebtToEbitda <= 0 ? 95 : d.netDebtToEbitda <= 1 ? 85 : d.netDebtToEbitda <= 2 ? 70 : d.netDebtToEbitda <= 3 ? 50 : 30, weight: 0.35, name: 'Net Debt/EBITDA' });
    if (r.currentRatio != null) financialStrengthSignals.push({ value: r.currentRatio >= 1.5 ? 85 : r.currentRatio >= 1 ? 65 : 35, weight: 0.25, name: 'Current Ratio' });
  }
  const fsWeight = financialStrengthSignals.reduce((sum, x) => sum + x.weight, 0);
  let financialStrength = fsWeight
    ? financialStrengthSignals.reduce((sum, x) => sum + x.value * x.weight, 0) / fsWeight
    : 50;
  const balanceSheetAvailable = Boolean(financials.rawAvailability?.annualBalanceSheet);
  const cashFlowAvailable = Boolean(financials.rawAvailability?.annualCashFlow);
  const financialStrengthCoverage = balanceSheetAvailable && cashFlowAvailable
    ? 'FULL'
    : balanceSheetAvailable || cashFlowAvailable
      ? 'PARTIAL'
      : 'CURRENT_FIELDS_ONLY';
  if (!balanceSheetAvailable) financialStrength = Math.min(financialStrength, 70);
  if (!cashFlowAvailable) financialStrength = Math.min(financialStrength, 75);
  if (sectorFramework?.coverage != null && sectorFramework.coverage < 50) financialStrength = Math.min(financialStrength, 65);
  financialStrength = clamp(financialStrength);

  // Wealth creation = sustainable returns + cash generation + growth.
  let wealth = 50;
  if (r.roe != null) wealth += r.roe >= 20 ? 15 : r.roe >= 15 ? 10 : r.roe >= 10 ? 5 : 0;
  if (d.fcfConversion != null) wealth += d.fcfConversion >= 90 ? 12 : d.fcfConversion >= 70 ? 8 : d.fcfConversion >= 50 ? 4 : -5;
  if (g.eps5yCagr != null) wealth += g.eps5yCagr >= 15 ? 13 : g.eps5yCagr >= 10 ? 8 : g.eps5yCagr >= 5 ? 4 : 0;
  wealth = clamp(wealth);

  let valuationScore = 50;
  if (valuation.trailingPE != null && valuation.trailingPE > 0) {
    valuationScore = clamp(100 - Math.max(0, valuation.trailingPE - 15) * 2.2);
  }
  if (valuation.marginOfSafety != null) valuationScore = clamp(valuationScore + valuation.marginOfSafety * 0.8);

  // A perfect valuation score is not justified by P/E alone. Require a
  // defensible fair value and at least one supporting valuation cross-check.
  const valuationInputs = [
    valuation.trailingPE, valuation.forwardPE, valuation.priceToBook,
    valuation.fairValue, valuation.growthSignal, valuation.evToEbitda
  ];
  const valuationAvailable = valuationInputs.filter(v => v != null).length;
  if (valuation.fairValue == null) valuationScore = Math.min(valuationScore, 65);
  else if (valuationAvailable < 3) valuationScore = Math.min(valuationScore, 75);
  else if (valuationAvailable < 4) valuationScore = Math.min(valuationScore, 85);

  let technicalScore = 50;
  if (technical.trend === 'STRONG UPTREND') technicalScore += 25;
  else if (technical.trend === 'UPTREND') technicalScore += 15;
  else if (technical.trend === 'DOWNTREND') technicalScore -= 20;
  if (technical.rsi != null) {
    if (technical.rsi >= 70) technicalScore -= 10;
    else if (technical.rsi >= 50) technicalScore += 8;
    else if (technical.rsi < 30) technicalScore += 4;
  }
  technicalScore = clamp(technicalScore);

  let risk = 35;
  if (sectorFramework?.key === 'BANKING') {
    if (r.roa != null && r.roa < 0.8) risk += 15;
    if (r.roe != null && r.roe < 10) risk += 10;
    if ((sectorFramework.coverage ?? 0) < 50) risk += 12;
  } else if (r.debtToEquity != null && r.debtToEquity > 1.5) {
    risk += 20;
  }
  if (valuation.trailingPE != null && valuation.trailingPE > 40) risk += 18;
  if (technical.trend === 'DOWNTREND') risk += 12;
  if (technical.distanceFrom200DMA != null && technical.distanceFrom200DMA < -20) risk += 10;
  if (technical.drawdown != null && technical.drawdown > 30) risk += 8;
  if (quality.completeness < 70) risk += 15;
  risk = clamp(risk);

  const w = SCORE_MODEL.weights;
  const componentValues = {
    quality: q,
    growth,
    wealth,
    financialStrength,
    valuation: valuationScore,
    technical: technicalScore,
    inverseRisk: 100 - risk,
    confidence: evidenceConfidence,
  };
  const components = Object.entries(componentValues).map(([key, value]) => ({
    key,
    value: Math.round(value),
    weight: w[key],
    contribution: Number((value * w[key]).toFixed(2)),
  }));
  const overall = clamp(components.reduce((sum, item) => sum + item.contribution, 0));
  const dataLimited = evidenceConfidence < 70 || quality.criticalMissingFields?.length > 0 || financialStrengthCoverage !== 'FULL';
  const classification = overall >= 90 ? 'EXCEPTIONAL' : overall >= 80 ? 'STRONG' : overall >= 70 ? 'PROMISING / WATCH' : overall >= 60 ? 'MIXED' : 'WEAK / HIGH RISK';

  return {
    quality: Math.round(q),
    wealth: Math.round(wealth),
    growth: Math.round(growth),
    financialStrength: Math.round(financialStrength),
    financialStrengthRaw: Math.round(financialStrength),
    financialStrengthCoverage,
    financialStrengthEvidence: financialStrengthSignals.map(({ name }) => name),
    valuation: Math.round(valuationScore),
    technical: Math.round(technicalScore),
    risk: Math.round(risk),
    confidence: Math.round(evidenceConfidence),
    overall: Math.round(overall),
    classification,
    dataLimited,
    sectorFramework: sectorFramework || null,
    components,
    model: SCORE_MODEL,
  };
}

function decision(score, valuation, technical, quality) {
  const reasons = [];
  const positives = [];
  const blockers = [];

  if (score.quality >= 75) positives.push(`Business quality is ${score.quality}/100.`);
  if (score.growth >= 65) positives.push(`Growth/wealth creation is ${score.growth}/100.`);
  if (score.financialStrength >= 70) positives.push(`Financial strength is ${score.financialStrength}/100 (${score.financialStrengthCoverage}).`);
  if (valuation.verdict === 'UNDERVALUED' || valuation.verdict === 'ATTRACTIVE') positives.push(`Valuation is ${valuation.verdict.toLowerCase()} under the current framework.`);
  if (technical.trend === 'STRONG UPTREND' || technical.trend === 'UPTREND') positives.push(`Technical trend is ${technical.trend.toLowerCase()}.`);

  if (quality.confidence < 70) blockers.push(`Evidence confidence is only ${quality.confidence}/100.`);
  if (quality.criticalMissingFields?.length) blockers.push(`Critical data is missing: ${quality.criticalMissingFields.join(', ')}.`);
  if (score.financialStrengthCoverage !== 'FULL') blockers.push(`Financial-statement coverage is ${score.financialStrengthCoverage}.`);
  if (quality.sectorFramework?.coverage != null && quality.sectorFramework.coverage < 50) blockers.push(`Sector KPI coverage is only ${quality.sectorFramework.coverage}%; key industry metrics remain unavailable.`);
  if (valuation.verdict === 'EXPENSIVE' || valuation.verdict === 'VERY EXPENSIVE') blockers.push(`Valuation is ${valuation.verdict.toLowerCase()}.`);
  if (score.risk >= 55) blockers.push(`Risk score is ${score.risk}/100.`);
  if (technical.trend === 'DOWNTREND') blockers.push('Technical trend is in a downtrend.');

  if (quality.confidence < 60) {
    return {
      action: 'DATA INSUFFICIENT',
      reason: [...blockers, 'Do not make a capital-allocation decision until core evidence improves.'].slice(0, 5),
      positives,
      blockers,
      basis: { score: score.overall, confidence: quality.confidence, valuation: valuation.verdict, trend: technical.trend },
    };
  }

  if (valuation.verdict === 'VERY EXPENSIVE' && score.overall >= 68) {
    return {
      action: 'WAIT / WATCH',
      reason: [...positives.slice(0, 2), ...blockers.slice(0, 2), 'Wait for a better entry price or stronger earnings evidence.'].slice(0, 5),
      positives,
      blockers,
      basis: { score: score.overall, confidence: quality.confidence, valuation: valuation.verdict, trend: technical.trend },
    };
  }

  if (score.overall >= 78 && score.risk < 40 && technical.trend !== 'DOWNTREND' &&
      valuation.verdict !== 'EXPENSIVE' && valuation.verdict !== 'VERY EXPENSIVE' && quality.confidence >= 70) {
    return {
      action: 'BUY / ACCUMULATE',
      reason: [...positives.slice(0, 3), 'Risk and evidence quality clear the current framework threshold.'].slice(0, 5),
      positives,
      blockers,
      basis: { score: score.overall, confidence: quality.confidence, valuation: valuation.verdict, trend: technical.trend },
    };
  }

  const technicallyWeak = technical.trend === 'DOWNTREND' || (technical.distanceFrom200DMA != null && technical.distanceFrom200DMA < -20);
  if (score.overall >= 68 && score.risk < 55 && !technicallyWeak) {
    return {
      action: 'BUY ON WEAKNESS / HOLD',
      reason: [...positives.slice(0, 3), ...blockers.slice(0, 2), 'Use price confirmation and margin of safety before adding aggressively.'].slice(0, 5),
      positives,
      blockers,
      basis: { score: score.overall, confidence: quality.confidence, valuation: valuation.verdict, trend: technical.trend },
    };
  }

  if (score.overall >= 60 && technicallyWeak) {
    return {
      action: 'WAIT / WATCH',
      reason: [...positives.slice(0, 2), 'Price is materially below the long-term trend reference.', 'Wait for technical recovery or a stronger margin of safety.'].slice(0, 5),
      positives,
      blockers,
      basis: { score: score.overall, confidence: quality.confidence, valuation: valuation.verdict, trend: technical.trend },
    };
  }

  if (score.overall < 48 || score.risk >= 70) {
    return {
      action: 'AVOID / REVIEW',
      reason: [...blockers.slice(0, 3), 'Current risk/reward does not clear the framework threshold.'].slice(0, 5),
      positives,
      blockers,
      basis: { score: score.overall, confidence: quality.confidence, valuation: valuation.verdict, trend: technical.trend },
    };
  }

  return {
    action: 'WAIT / WATCH',
    reason: [...positives.slice(0, 2), ...blockers.slice(0, 2), 'Wait for better valuation, improving fundamentals, or technical confirmation.'].slice(0, 5),
    positives,
    blockers,
    basis: { score: score.overall, confidence: quality.confidence, valuation: valuation.verdict, trend: technical.trend },
  };
}

function buildMetricProvenance(financials, valuation, technical, raw, marketAsOf, fundamentalsAsOf) {
  const definitions = METRIC_DEFINITIONS;
  const out = {};
  const add = (key, value, source, period, asOf, status = null) => {
    const definition = definitions[key] || null;
    out[key] = {
      source,
      period,
      asOf: asOf || null,
      status: status || (value == null ? 'NOT_AVAILABLE' : definition?.sourceType === 'calculated' ? 'CALCULATED' : 'PROVIDER'),
      unit: definition?.unit || null,
      formula: definition?.formula || null,
    };
  };
  add('currentPrice', valuation.currentPrice, 'Yahoo Finance chart', 'latest daily close/market price', marketAsOf);
  add('trailingPE', valuation.trailingPE, 'Yahoo Finance quoteSummary.summaryDetail', 'current provider field', fundamentalsAsOf);
  add('forwardPE', valuation.forwardPE, 'Yahoo Finance quoteSummary.summaryDetail/defaultKeyStatistics', 'forward provider field', fundamentalsAsOf);
  add('priceToBook', valuation.priceToBook, 'Yahoo Finance quoteSummary.defaultKeyStatistics', 'current provider field', fundamentalsAsOf);
  add('roe', financials.ratios.roe, 'Yahoo Finance quoteSummary.financialData', 'current provider field', fundamentalsAsOf);
  add('debtToEquity', financials.ratios.debtToEquity, 'Yahoo Finance quoteSummary.financialData', 'current provider field', fundamentalsAsOf);
  add('revenueGrowth', financials.ratios.revenueGrowth, 'Yahoo Finance quoteSummary.financialData', 'current provider field', fundamentalsAsOf);
  add('revenue5yCagr', financials.growth.revenue5yCagr, 'StockSamjho deterministic engine', 'annual 12M history', fundamentalsAsOf);
  add('eps5yCagr', financials.growth.eps5yCagr, 'StockSamjho deterministic engine', 'annual 12M history', fundamentalsAsOf);
  add('fcfConversion', financials.derived.fcfConversion, 'StockSamjho deterministic engine', 'current/TTM provider inputs', fundamentalsAsOf);
  add('fairValue', valuation.fairValue, 'StockSamjho deterministic engine', 'current framework estimate', fundamentalsAsOf);
  add('valuationVerdict', valuation.verdict, 'StockSamjho deterministic engine', 'current framework', fundamentalsAsOf);
  add('sma200', technical.s200, 'StockSamjho deterministic engine', 'daily close history', marketAsOf);
  add('ema200', technical.e200, 'StockSamjho deterministic engine', 'daily close history', marketAsOf);
  add('rsi14', technical.rsi, 'StockSamjho deterministic engine', 'daily close history', marketAsOf);
  add('atr14', technical.atr, 'StockSamjho deterministic engine', 'daily OHLC history', marketAsOf);
  add('relativeVolume', technical.relativeVolume, 'StockSamjho deterministic engine', 'latest daily volume vs 20D average', marketAsOf);
  return out;
}

function sanitize(value) {
  if (value === undefined) return null;
  if (typeof value === 'number' && !Number.isFinite(value)) return null;
  if (Array.isArray(value)) return value.map(sanitize);
  if (value && typeof value === 'object') {
    const out={};
    for (const [k,v] of Object.entries(value)) out[k]=sanitize(v);
    return out;
  }
  return value;
}
export async function analyze(input, { marketOverride = null } = {}) {
  const symbol = ticker(input);

  const [marketResult, fundamentalsResult, statementEvidenceResult] = await Promise.allSettled([
    marketOverride ? Promise.resolve(marketOverride) : fetchChart(symbol),
    fetchFundamentals(symbol),
    fetchStatementEvidence(symbol),
  ]);

  if (marketResult.status !== 'fulfilled') {
    throw marketResult.reason;
  }

  const market = marketResult.value;
  const raw = fundamentalsResult.status === 'fulfilled'
    ? fundamentalsResult.value
    : { summary: null, annual: [], trailing: [], errors: { fatal: String(fundamentalsResult.reason?.message || fundamentalsResult.reason) } };

  const t = technical(market.rows);
  const statementEvidence = statementEvidenceResult.status === 'fulfilled'
    ? statementEvidenceResult.value
    : null;
  const financials = mergeStatementEvidence(buildFinancials(raw), statementEvidence);
  const valuation = buildValuation(raw, market.price);
  const ownership = buildOwnership(raw);
  const profile = raw.summary?.assetProfile || {};
  const quoteType = raw.summary?.quoteType || {};
  const sectorFramework = buildSectorFramework(financials, profile.sector, profile.industry);
  const marketAsOf = market.rows.at(-1)?.date || null;
  const summaryPriceTime = num(raw.summary?.price?.regularMarketTime) || null;
  const fundamentalsAsOf = summaryPriceTime ? new Date(summaryPriceTime * 1000).toISOString() : raw.fetchedAt || null;
  const dataQuality = buildDataQuality(financials, valuation, ownership, t, { marketAsOf, fundamentalsAsOf }, sectorFramework);
  const score = scoreStock(financials, valuation, t, dataQuality, sectorFramework);
  const finalDecision = decision(score, valuation, t, dataQuality);

  const stock = {
    symbol: String(input).toUpperCase().replace(/\.NS$|\.BO$/i, ''),
    yahooSymbol: symbol,
    name: quoteType.longName || quoteType.shortName || symbol,
    sector: profile.sector ?? null,
    industry: profile.industry ?? null,
    price: market.price,
    currency: market.currency,
    exchange: market.exchange,
    marketCap: valuation.marketCap,
    pe: valuation.trailingPE,
    pb: valuation.priceToBook,
    roe: financials.ratios.roe,
    roce: financials.derived.roceFromStatements,
    de: financials.ratios.debtToEquity,
    fcfRaw: financials.current.freeCashFlow,
    fcfCrore: crore(financials.current.freeCashFlow),
    npm: financials.ratios.netMargin,
    opm: financials.ratios.operatingMargin,
    rg: financials.ratios.revenueGrowth,
    eg: financials.ratios.earningsGrowth,
    // The frontend labels this field in ₹ Cr, so expose the normalized crore value.
    fcf: crore(financials.current.freeCashFlow),
    low: t.low52Week,
    high: t.high52Week,
    dataNote: 'Live market data + Yahoo Finance quoteSummary/fundamentalsTimeSeries. Missing values are reported as null rather than invented.',
    modelVersion: 'analysis-v1.0',
    scoreClassification: score.classification,
    dataLimited: score.dataLimited,
    sectorFramework: sectorFramework.key,
  };

  const response = {
    stock,
    score,
    fundamentals: financials,
    valuation,
    ownership,
    technical: t,
    dataQuality,
    decision: finalDecision,
    source: 'Yahoo Finance via yahoo-finance2 (normalized)',
    asOf: new Date().toISOString(),
    metricDefinitions: METRIC_DEFINITIONS,
    sectorFramework,
    provenance: {
      metrics: buildMetricProvenance(financials, valuation, t, raw, marketAsOf, fundamentalsAsOf),
      marketData: { source: 'Yahoo Finance chart', interval: '1d', asOf: marketAsOf, currency: market.currency, validation: 'normalized' },
      currentFundamentals: { source: 'Yahoo Finance quoteSummary.financialData/defaultKeyStatistics', period: 'provider-reported current/TTM fields', asOf: summaryPriceTime ? new Date(summaryPriceTime * 1000).toISOString() : null, validation: 'normalized; provider schema validation disabled' },
      annualFundamentals: { source: 'Yahoo Finance fundamentalsTimeSeries', period: '12M/annual', historyRows: financials.periods.annualHistoryCount, validation: 'normalized' },
      calculatedMetrics: { source: 'StockSamjho deterministic engine', formulaRegistry: 'core-metrics-v1.0' },
    metricDefinitions: 'core-metrics-v1.0',
      score: { model: SCORE_MODEL.id, status: SCORE_MODEL.status },
    },
  };
  return sanitize(response);
}

export { atr, rsi, normalizeDebtToEquity, technical, buildFinancials, buildValuation, buildDataQuality, scoreStock, decision, SCORE_MODEL, buildSectorFramework, classifySector };
