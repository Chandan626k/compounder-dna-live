...
function buildFinancials(raw) {
  ...
  const netDebt = metricRows(usableFinancialRows, 'netDebt');
...
function buildDataQuality(financials, valuation, ownership, technical) {
  const checks = [
    ['price', valuation.currentPrice, 1],
    ['P/E', valuation.trailingPE, 1],
    ['Forward P/E', valuation.forwardPE, 0.5],
    ['P/B', valuation.priceToBook, 0.5],
    ['Fair Value', valuation.fairValue, 1],
    ['Growth Signal', valuation.growthSignal, 1],
    ['ROE', financials.ratios.roe, 1],
    ['Debt/Equity', financials.ratios.debtToEquity, 1],
    ['Net Margin', financials.ratios.netMargin, 1],
    ['Operating Margin', financials.ratios.operatingMargin, 0.5],
    ['Revenue Growth', financials.ratios.revenueGrowth, 1],
    ['Earnings Growth', financials.ratios.earningsGrowth, 1],
    ['Free Cash Flow', financials.current.freeCashFlow, 1],
    ['5Y Revenue CAGR', financials.growth.revenue5yCagr, 1],
    ['5Y EPS CAGR', financials.growth.eps5yCagr, 1],
    ['RSI', technical.rsi, 0.5],
    ['200 DMA', technical.s200, 1],
  ];
  const totalWeight = checks.reduce((s, x) => s + x[2], 0);
  const availableWeight = checks.filter(([,v]) => v != null).reduce((s,x) => s+x[2], 0);
  let completeness = Math.round((availableWeight / totalWeight) * 100);
  const missing = checks.filter(([,v]) => v == null).map(([name]) => name);
  const warnings = [
  ...
  const verifiedFields = ['price','P/E','P/B','ROE','Debt/Equity','Net Margin','Operating Margin','Revenue Growth','Earnings Growth','Free Cash Flow','RSI','200 DMA'].filter(name => !missing.includes(name));
  const calculatedFields = ['5Y Revenue CAGR','5Y EPS CAGR','Growth Signal','Fair Value'].filter(name => !missing.includes(name));
  return {
    completeness,
    confidence: completeness,
    available: checks.filter(([,v]) => v != null).length,
    total: checks.length,
    verifiedFields,
    calculatedFields,
