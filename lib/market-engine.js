...
function buildDataQuality(financials, valuation, ownership, technical) {
  ...
  const warnings = [
    ...
    ...validationFlags,
    ...Object.values(financials.errors || {}).filter(Boolean).map((x) => `Provider error: ${x}`),
  ];
...
function scoreStock(financials, valuation, technical, quality) {
  ...
  const overall = clamp(
    ...
    growth * w.growth +
    wealth * w.wealth +
    financialStrength * w.financialStrength +
    valuationScore * w.valuation +
    technicalScore * w.technical +
    (100 - risk) * w.inverseRisk +
    quality.confidence * w.confidence
  );
