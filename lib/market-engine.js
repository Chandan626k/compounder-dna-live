...
function buildDataQuality(financials, valuation, ownership, technical, freshness = {}) {
  const checks = [
...
function scoreStock(financials, valuation, technical, quality) {
  ...
  if (r.debtToEquity != null) {
    financialStrengthSignals.push({
      value: r.debtToEquity <= 0.25 ? 90 : r.debtToEquity <= 0.75 ? 78 : r.debtToEquity <= 1.5 ? 60 : r.debtToEquity <= 2.5 ? 42 : 25,
      weight: 0.40,
      name: 'Debt/Equity',
    });
  }
  if (d.netDebtToEbitda != null) {
    financialStrengthSignals.push({
      value: d.netDebtToEbitda <= 0 ? 95 : d.netDebtToEbitda <= 1 ? 85 : d.netDebtToEbitda <= 2 ? 70 : d.netDebtToEbitda <= 3 ? 50 : 30,
      weight: 0.35,
  ...
  if (r.currentRatio != null) {
    financialStrengthSignals.push({
      value: r.currentRatio >= 1.5 ? 85 : r.currentRatio >= 1 ? 65 : 35,
      weight: 0.25,
      name: 'Current Ratio',
    });
  }
...
export async function analyze(input) {
  ...
  const dataQuality = buildDataQuality(financials, valuation, ownership, t, { marketAsOf, fundamentalsAsOf });
...
export { atr, rsi, normalizeDebtToEquity, technical, buildFinancials, buildValuation, buildDataQuality, scoreStock, decision, SCORE_MODEL };
