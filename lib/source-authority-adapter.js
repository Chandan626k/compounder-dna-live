...
export function qualifySourceArtifact({
  ...
  const productionAuthority = snapshot?.accessMode === SOURCE_ACCESS_MODES.AUTHORITATIVE &&
  ...
  if (!provenanceVerified || !productionAuthority) return { stage: 'SEMANTICALLY_VALIDATED', status: 'NOT_READY', historicalValidity: historical.status, reason: 'Production provenance/authority is not established.' };
...
export function isProductionAuthoritative(snapshot) {
  return snapshot?.accessMode === SOURCE_ACCESS_MODES.AUTHORITATIVE && Boolean(snapshot?.authorityClass);
}
