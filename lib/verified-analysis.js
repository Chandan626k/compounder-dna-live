...
const latestEvidenceDate = (evidence) => {
  const dates = [...(evidence?.income || []), ...(evidence?.balance || []), ...(evidence?.cash || [])].map(row => row?.date).map(ISO).filter(Boolean).sort();
...
export async function analyzeVerified(input) {
  ...
  const currentQuote = quoteResult.status === 'fulfilled' ? quoteResult.value : null;
  ...
  if (!statementEvidence) {
    ...
    catch (error) { statementEvidence = { provider: 'Yahoo Finance fundamentalsTimeSeries', income: [], balance: [], cash: [], evidence: { balance: {}, cash: {} }, coverage: { income: false, balanceSheet: false, cashFlow: false }, history: { incomeYears: 0, balanceYears: 0, cashFlowYears: 0 }, errors: { provider: String(error?.message || error) }, fetchedAt: new Date().toISOString() }; }
  }
  ...
  const verifiedDailyClose = verifiedTechnical.last;
  ...
  financials = { ...financials, errors: { ...(financials.errors || {}), ...Object.fromEntries(Object.entries(statementEvidence.errors || {}).map(([key, value]) => [`statements.${key}`, value]).filter(([, value]) => Boolean(value))) }, periods: { ...(financials.periods || {}), annualHistoryCount: Math.max(Number(financials.periods?.annualHistoryCount || 0), Number(statementEvidence.history?.incomeYears || 0)) } };
  const currentPrice = Number.isFinite(currentQuote?.price) && currentQuote.price > 0 ? currentQuote.price : verifiedDailyClose;
  const priceSource = Number.isFinite(currentQuote?.price) && currentQuote.price > 0 ? 'CURRENT_PROVIDER_QUOTE' : 'LATEST_VERIFIED_DAILY_BAR';
  const valuation = recalculateValuationAtPrice(base?.valuation || fallbackValuation, currentPrice);
  ...
  return {
    ...(base || {}),
    stock: { ...stock, price: currentPrice, marketCap: valuation.marketCap, dataNote: 'Verified daily market history + provider fundamentals. Current quote is kept separate from daily technical evidence; missing values remain null.' },
    fundamentals: financials, valuation, ownership, technical: verifiedTechnical, score, decision: finalDecision, dataQuality, currentQuote,
    provenance: {
      ...
      marketData: { ...(base?.provenance?.marketData || {}), source: 'Yahoo Finance chart API via verifiedHistory', asOf: marketAsOf, retrievedAt: history.retrievedAt || null, interval: '1d', validation: 'verified provider payload + OHLCV integrity checks', status: history.status || 'PRIMARY', observationType: 'LATEST_DAILY_BAR' },
      currentQuote: currentQuote ? { source: currentQuote.provider || 'Yahoo Finance quote API', price: currentQuote.price, observedAt: currentQuote.observedAt, ageSeconds: currentQuote.ageSeconds, retrievedAt: currentQuote.retrievedAt, marketState: currentQuote.marketState, exchangeDataDelayedBy: currentQuote.exchangeDataDelayedBy, quoteSourceName: currentQuote.quoteSourceName, status: currentQuote.status } : null,
      priceSelection: { source: priceSource, dailyBarClose: verifiedDailyClose, currentQuotePrice: currentQuote?.price ?? null },
