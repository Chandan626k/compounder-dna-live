...
export function buildVerifiedDataQualityProvenance({ dataQuality, marketAsOf, fundamentalsAsOf }) {
  ...
  return { asOf, freshness: freshnessLabel(freshnessScore), freshnessScore, coveragePct, confidence: Number.isFinite(dataQuality?.confidence) ? dataQuality.confidence : 0, provider: 'Yahoo Finance', source: 'Yahoo Finance chart API + quoteSummary + fundamentalsTimeSeries' };
}
...
export async function analyzeVerified(input) {
  ...
  const provenanceDataQuality = buildVerifiedDataQualityProvenance({ dataQuality: dataQualityBase, marketAsOf, fundamentalsAsOf });
  ...
  return {
    ...
    provenance: {
      ...
      marketData: { ...(base?.provenance?.marketData || {}), source: 'Yahoo Finance chart API via verifiedHistory', asOf: marketAsOf, interval: '1d', validation: 'verified provider payload', status: history.status || 'PRIMARY' },
