...
export async function analyzeVerified(input) {
  ...
  const symbol = requestedSymbol.startsWith('^') || requestedSymbol.endsWith('.NS') || requestedSymbol.endsWith('.BO')
    ? requestedSymbol
    : `${requestedSymbol}.NS`;

  let history;
  try {
    history = await verifiedHistory(symbol, { interval: '1d', days: 900, minBars: 60 });
  } catch (error) {
    throw new Error(`VERIFIED_MARKET_DATA_UNAVAILABLE:${error?.message || error}`);
  }
  ...
  if (!statementEvidence) {
    try {
      statementEvidence = await fetchStatementEvidence(symbol);
    } catch (error) {
      statementEvidence = {
        ...
        errors: { provider: String(error?.message || error) },
  ...
  const fallbackStock = {
    ...
    currency: 'INR',
    exchange: null,
    marketCap: fallbackValuation.marketCap,
    dataNote: 'Verified market history + provider statements only. Quote-summary fields unavailable remain null.',
    modelVersion: 'analysis-v1.0',
    dataLimited: true,
    sectorFramework: 'DEFAULT',
  };

  const verifiedTechnical = technical(rows);
  const verifiedPrice = verifiedTechnical.last;
  const mergedFinancials = mergeStatementEvidence(base?.fundamentals || fallbackFinancials, statementEvidence);
  const financials = {
    ...mergedFinancials,
    errors: {
