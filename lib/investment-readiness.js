...
function scoreValuation(v = {}, price = null, valuationAuthority = null) {
  ...
  if (price != null && n(v.fairValue) != null && n(v.fairValue) > 0) {
    const derivedMos = (1 - price / v.fairValue) * 100;
    return { score: clamp(50 + derivedMos * 1.5), basis: 'verified fair value + current price', marginOfSafety: derivedMos, verdict: v.verdict ?? null };
  }
...
export function buildInvestmentReadiness(analysis) {
  ...
  // The canonical analysis contract exposes fundamentals, while older callers
  // may still provide financials. Prefer the canonical field and retain the
  // compatibility fallback so the readiness layer cannot silently score an
  // otherwise valid analysis as data-missing.
  ...
  // buildDataQuality calls this field `completeness`; accept `coverage` only as
  // a backward-compatible alias. Do not treat an absent coverage signal as 0.
  ...
  const valuationAuthority = analysis?.valuationEvidenceAuthority || resolveValuationEvidence({
    valuation,
    valuationMetricLineage: valuation.metricLineage,
    financials,
    ticker: analysis?.stock?.yahooSymbol || analysis?.symbol || analysis?.ticker || null,
    issuer: analysis?.stock?.issuer || null,
    currentPriceEvidence: valuation.metricLineage?.currentPrice?.evidence || null,
    valuationMethod: valuation.metricLineage?.valuationMethod || 'DEFAULT',
  });
  ...
  return {
    ...
    verifiedEvidence: {
      ...
      valuationEvidenceStatus: valuationAuthority.valuationEvidenceStatus,
      valuationVerdict: v.verdict,
      marginOfSafety: v.marginOfSafety,
      price,
    },
    valuationEvidenceAuthority: valuationAuthority,
    checks: f.checks,
    blockers,
