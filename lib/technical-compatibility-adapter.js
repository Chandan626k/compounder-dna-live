...
function legacyTrend(canonical) {
  ...
  return e50 != null && last > e50 ? 'UPTREND' : 'RECOVERING / MIXED';
}
function legacySupportResistance(rows) {
  if (rows.length < 20) return { support: null, resistance: null };
  const recent = rows.slice(-20);
  return { support: Math.min(...recent.map((row) => row.low)), resistance: Math.max(...recent.map((row) => row.high)) };
}
function legacy52Week(rows) {
  if (rows.length < 252) return { has52WeekHistory: false, high: null, low: null };
  const year = rows.slice(-252);
  return { has52WeekHistory: true, high: Math.max(...year.map((row) => row.high)), low: Math.min(...year.map((row) => row.low)) };
}
function legacyVolumeTrend(rows) {
  if (rows.length < 40) return null;
  const volumes = rows.map((row) => row.volume), current = volumes.slice(-20).reduce((sum, value) => sum + value, 0) / 20, previous = volumes.slice(-40, -20).reduce((sum, value) => sum + value, 0) / 20;
 ...
 * Legacy field semantics are preserved while canonical provenance/validation
 * remains authoritative for the underlying evidence.
 */
export function technicalCompatibility(rows, context = {}) {
  ...
  return {
    ...
    rangePosition: high != null && low != null && high > low ? ((canonical.last - low) / (high - low)) * 100 : null,
    provenance: {
