...
import {
  ...
  expectedTrainingWindowDefinition,
  expectedOOSWindowDefinition,
  finalCandidateIndependenceStatus,
...
export function buildEvidenceProvenance({ artifact, candidateId, candidateParameters, evidenceType, researchRows }) {
  ...
  if (!['MODEL_SELECTION_EVIDENCE','FINAL_CANDIDATE_VALIDATION_EVIDENCE'].includes(evidenceType)) return { status: 'UNVERIFIABLE', reason: 'UNKNOWN_EVIDENCE_TYPE' };

  const selected = artifact.modelSelection;
  const params = candidateParameters ?? selected.candidateParameters;
  const trainingWindowDefinition = expectedTrainingWindowDefinition(evidenceType, artifact);
  const OOSWindowDefinition = expectedOOSWindowDefinition(evidenceType, artifact);
  ...
  const independenceStatus = evidenceType === 'MODEL_SELECTION_EVIDENCE'
    ? 'VERIFIED'
    : finalCandidateIndependenceStatus(artifact);
  ...
  if (missing.length) return { ...provenance, status: 'UNVERIFIABLE', missing };
  if (evidenceType === 'FINAL_CANDIDATE_VALIDATION_EVIDENCE' && independenceStatus !== 'VERIFIED') {
    return { ...provenance, status: 'UNVERIFIABLE', reason: 'INDEPENDENT_FINAL_CANDIDATE_VALIDATION_UNAVAILABLE' };
  }
  return { ...provenance, status: 'VERIFIABLE' };
}
...
export function verifyEvidenceProvenance(artifact, evidence) {
  ...
  const expectedTraining = expectedTrainingWindowDefinition(evidence.evidenceType, artifact);
  const expectedOOS = expectedOOSWindowDefinition(evidence.evidenceType, artifact);
  ...
  const expectedIndependence = evidence.evidenceType === 'MODEL_SELECTION_EVIDENCE' ? 'VERIFIED' : finalCandidateIndependenceStatus(artifact);
  ...
  const checks = {
    ...
    independenceStatus: evidence.independenceStatus === expectedIndependence && (evidence.evidenceType === 'MODEL_SELECTION_EVIDENCE' || expectedIndependence === 'VERIFIED'),
  };
...
function provenanceState(result) {
  ...
  return {
    ...
    finalCandidateIndependence: finalCandidate?.provenance?.independenceStatus ?? 'UNAVAILABLE',
...
export function buildValidationGate(result = {}) {
  ...
  if (provenance.finalCandidateIndependence !== 'VERIFIED') blockers.push('FINAL_CANDIDATE_INDEPENDENCE_UNVERIFIED');
  ...
  return {
    ...
    nextEvidence: productionEligible ? [] : [
      ...
      'Provide a genuinely independent pre-holdout final-candidate evaluation partition or a pre-authorized post-selection inference method.',
    ],
  };
}
