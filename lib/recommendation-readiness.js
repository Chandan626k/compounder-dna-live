...
function buildLongTerm(analysis) {
  ...
  const unverified = requirements.filter((r) => r.available && !['VERIFIED','CALCULATED','PROVIDER','AVAILABLE'].includes(r.status));
...
function buildSwing(analysis, trading) {
  ...
  const allVerified = requirements.every((r) => r.available && ['VERIFIED','AVAILABLE'].includes(r.status))
    && lifecycle != null
