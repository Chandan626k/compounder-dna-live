...
function buildLongTerm(analysis) {
  ...
  const pitBlocked = records.some((r) => ['PIT_INELIGIBLE','SUPERSEDED','INVALID'].includes(r.pitEligibility));
  ...
  if (pitBlocked) blockers.push({ key: 'pit', reason: 'One or more financial evidence records are not eligible for the evaluation point in time.', status: 'BLOCKED' });
  ...
  return {
    ...
    missingEvidence,
    blockers,
    evidenceStatus: { currency: currencyStatus, pit: pitBlocked ? 'BLOCKED' : pitUnknown ? 'UNKNOWN' : 'NOT_ESTABLISHED' },
