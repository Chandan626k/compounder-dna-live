...
function normalizeProviderError(error) {
  ...
  const mapped = code.includes('AUTH') ? 'AUTHENTICATION_FAILURE'
...
function createAdapter({ exchange, source, authorityClass, transport = null, normalizeReporting, normalizeCalendar, config = {} }) {
  ...
  return Object.freeze({
    ...
    async fetchReportingArtifacts(request = {}) {
      ...
      try {
        return await transport.request({ kind: 'REPORTING', exchange, ...request, config: safeConfig });
      } catch (error) {
    ...
    async fetchCalendarArtifacts(request = {}) {
      requireTransport(transport);
      try {
        return await transport.request({ kind: 'CALENDAR_SESSION', exchange, ...request, config: safeConfig });
      } catch (error) {
        throw normalizeProviderError(error);
      }
    },
    normalizeReportingArtifact(artifact, metadata = {}) {
      ...
      try {
        return normalizeReporting(artifact, { exchange, source, authorityClass, ...metadata });
      } catch (error) {
    ...
    normalizeCalendarArtifact(artifact, metadata = {}) {
      ...
      try {
        return normalizeCalendar(artifact, { exchange, source, authorityClass, ...metadata });
      } catch (error) {
        if (error instanceof SourceProviderError) throw error;
    ...
    snapshot(payload, metadata = {}) {
      return createImmutableSourceSnapshot({
        source,
        authorityClass,
        accessMode: SOURCE_ACCESS_MODES.AUTHORITATIVE,
        payload,
    ...
    toReportingEvent(input = {}) {
      return createReportingEvent({ source, authorityClass, exchange, accessMode: SOURCE_ACCESS_MODES.AUTHORITATIVE, ...input });
    },
    toCalendarSession(input = {}) {
      return createExchangeSession({ exchange, accessMode: SOURCE_ACCESS_MODES.AUTHORITATIVE, ...input });
    },
  });
}
...
export function createNseCalendarAdapter(options = {}) {
  return createNseReportingAdapter(options);
}

export function createBseCalendarAdapter(options = {}) {
  return createBseReportingAdapter(options);
}
