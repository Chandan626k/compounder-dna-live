...
import { metricCalculationVersion as canonicalMetricCalculationVersion, modelSelectionAggregationMethod, selectionRuleVersion } from './canonical-provenance.js';
...
function bonferroniCI(values,alpha=0.05,tests=RESEARCH_CANDIDATES.length){
  ...
  const m=mean(values),sd=stdev(values);
  const a=alpha/tests;
...
export function walkForwardCandidateSearch(rows,{horizon=20,costs,trainBars=500,testBars=125,holdoutFraction=.20,candidates=RESEARCH_CANDIDATES}={}){
  ...
  const holdoutRows=rows.slice(holdoutStart-1);
  ...
  while(testStart+testBars<=researchRows.length){
    const trainStart=Math.max(0,testStart-trainBars),trainEnd=testStart-1;
    const ranked=candidates.map(p=>({p,r:runValidationBacktest(rows,{horizon,params:p,costs,start:trainStart,end:trainEnd})})).map(x=>({...x,score:scoreTraining(x.r)})).sort((a,b)=>(b.score.eligible-a.score.eligible)||(b.score.expectancy??-Infinity)-(a.score.expectancy??-Infinity));
    const chosen=ranked[0],testEnd=testStart+testBars-1;
  ...
  const usable=windows.filter(w=>w.test.trades>0),returns=usable.map(w=>w.test.totalReturnPct),exps=usable.map(w=>w.test.expectancyPct).filter(Number.isFinite),positive=usable.filter(w=>w.test.totalReturnPct>0).length;
  const method='WALK_FORWARD_MODEL_SELECTION';
  const selectionRule='training-only: >=8 trades, positive expectancy, PF>1, maxDD<=25%; otherwise highest training expectancy';
  const summary={windows:windows.length,usableWindows:usable.length,positiveWindowRatePct:usable.length?positive/usable.length*100:null,aggregateReturnPct:returns.reduce((s,x)=>s+x,0),meanTestExpectancyPct:mean(exps),testExpectancyBonferroniCI:bonferroniCI(exps),worstTestDrawdownPct:usable.length?Math.max(...usable.map(w=>w.test.maxDrawdownPct||0)):null};
  return {status:windows.length>=3?'COMPLETED':'INSUFFICIENT_WINDOWS',method,candidateCount:candidates.length,selectionRule,selectionRuleVersion:selectionRuleVersion(selectionRule),metricCalculationVersion:canonicalMetricCalculationVersion(runValidationBacktest.toString()),aggregationMethod:modelSelectionAggregationMethod({method,summaryKeys:Object.keys(summary).sort(),windows:windows.length}),windows,summary,finalHoldout:{status:'UNTOUCHED',startIndex:holdoutStart,bars:holdoutRows.length,selectionUsed:false,evaluated:false},decision:'HOLDOUT_REQUIRED'};
}

export function diagnoseFailure(strategy,benchmark){
  return {strategyVsBenchmark:{status:'NOT_COMPARABLE_YET',strategyReturnPct:null,benchmarkReturnPct:benchmark?.totalReturnPct??null,excessReturnPct:null,reason:'The benchmark shown is full-period buy-and-hold while the strategy diagnostic is rolling research-window output. A direct return comparison would be statistically invalid until both are evaluated over the identical frozen OOS interval.'},failureReasons:{negativeExpectancy:Number(strategy?.expectancyPct)<0,profitFactorBelowOne:Number(strategy?.profitFactor)<1,drawdownHigh:Number(strategy?.maxDrawdownPct)>25,lowPositiveWindows:Number(strategy?.positiveWindowRatePct)<50}};
}
