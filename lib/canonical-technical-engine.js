...
function structure(rows, lookback = 20) {
  if (rows.length < 4) return { state: 'INSUFFICIENT_DATA', lastEvent: null, breakout: null, breakdown: null }; const current = rows.at(-1), previous = rows.at(-2), prior = rows.slice(-Math.min(lookback + 1, rows.length), -1), priorHigh = Math.max(...prior.map((row) => row.high)), priorLow = Math.min(...prior.map((row) => row.low)); const higherHigh = current.high > previous.high, higherLow = current.low > previous.low, lowerHigh = current.high < previous.high, lowerLow = current.low < previous.low; const state = higherHigh && higherLow ? 'UPTREND_STRUCTURE' : lowerHigh && lowerLow ? 'DOWNTREND_STRUCTURE' : 'RANGE_OR_TRANSITION';
  return { state, lastEvent: higherHigh ? (higherLow ? 'HIGHER_HIGH_HIGHER_LOW' : 'HIGHER_HIGH') : lowerLow ? (lowerHigh ? 'LOWER_HIGH_LOWER_LOW' : 'LOWER_LOW') : higherLow ? 'HIGHER_LOW' : lowerHigh ? 'LOWER_HIGH' : null, breakout: current.close > priorHigh ? { confirmed: true, level: priorHigh, type: 'UPSIDE_CLOSE_ABOVE_PRIOR_LOOKBACK' } : null, breakdown: current.close < priorLow ? { confirmed: true, level: priorLow, type: 'DOWNSIDE_CLOSE_BELOW_PRIOR_LOOKBACK' } : null };
}
function supportResistance(rows) {
  if (rows.length < 20) return { support: null, resistance: null, levels: [], status: 'UNAVAILABLE' }; const recent = rows.slice(-20), recentLow = Math.min(...recent.map((row) => row.low)), recentHigh = Math.max(...recent.map((row) => row.high)), year = rows.slice(-Math.min(252, rows.length)), yearLow = Math.min(...year.map((row) => row.low)), yearHigh = Math.max(...year.map((row) => row.high)), levels = [...new Set([recentLow, recentHigh, yearLow, yearHigh].map((value) => Number(value.toFixed(6))))].sort((a, b) => a - b), price = rows.at(-1).close, supports = levels.filter((level) => level <= price), resistances = levels.filter((level) => level >= price); return { support: supports.at(-1) ?? null, resistance: resistances[0] ?? null, levels, status: 'VERIFIED_LEVELS' };
}
