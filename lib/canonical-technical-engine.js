...
function volumeTrend(volumes, period = 20) {
  if (!Array.isArray(volumes) || volumes.length < period * 2) return null;
  const current = sma(volumes.slice(-period), period), previous = sma(volumes.slice(-(period * 2), -period), period);
  return previous != null && previous > 0 && current != null ? ((current / previous) - 1) * 100 : null;
}
...
export function calculateCanonicalTechnical(inputRows, context = {}) {
  ...
  const rows = validation.rows, closes = rows.map((row) => row.close), volumes = rows.map((row) => row.volume), last = closes.at(-1), e20 = ema(closes, 20), e50 = ema(closes, 50), e100 = ema(closes, 100), e200 = ema(closes, 200), s20 = sma(closes, 20), s50 = sma(closes, 50), s200 = sma(closes, 200), rsi14 = rsi(closes, 14), atr14 = atr(rows, 14), macdValue = macd(closes), adx14 = adx(rows, 14), rv20 = relativeVolume(volumes, 20), volumeTrend20 = volumeTrend(volumes, 20), structureValue = structure(rows, 20), levels = supportResistance(rows), breakoutLifecycle = calculateCanonicalBreakoutLifecycle(rows, { ...context, timeframe, symbol, source: provenance.source, retrievedAt: provenance.retrievedAt, targetLevels: levels.zones, marketStructure: structureValue, supportResistance: levels });
