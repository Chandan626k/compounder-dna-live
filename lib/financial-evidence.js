...
export function buildEvidenceId({ source, ticker, sourceKey, reportingDate, periodType, statementScope }) {
  const identity = [source, ticker, sourceKey, reportingDate, periodType, statementScope].map(clean).join('|');
...
export function createCanonicalEvidence({
  ...
  return {
    evidenceId: buildEvidenceId({ source, ticker, sourceKey, reportingDate, periodType: normalizedPeriodType || periodType, statementScope }),
    source: clean(source),
