 ...
 * This function intentionally does NOT parse PDF bytes and does NOT OCR.
 * It accepts text only after an external extraction step and requires explicit
 * document identity + period + scope markers before any value can become VERIFIED.
 */
export function qualifyBankPdfText({
  issuer,
  ticker,
  documentType,
  documentTitle,
  ...
  const periodOk = has(text, [
    /quarter ended june 30,? 2026/i,
    /q1\s*fy\s*27/i,
    /q1[- ]2027/i,
  ]);
  if (!periodOk) errors.push('REPORTING_PERIOD_NOT_CONFIRMED_IN_EXTRACTED_TEXT');
  ...
  const qualifiedFields = Object.fromEntries(BANK_KPI_FIELDS.map((field) => [field, emptyField(errors.length ? 'UNVERIFIED' : 'UNVERIFIED')]));
  ...
  for (const field of BANK_KPI_FIELDS) {
    ...
    const unit = candidate.unit == null ? null : String(candidate.unit);
    const fieldText = normalize(candidate.context || '');
    if (!Number.isFinite(value) || !unit || !fieldText) continue;
    if (!fieldText || !text.includes(fieldText)) continue;
    ...
    qualifiedFields[field] = {
      ...
      provenance: errors.length ? null : {
        source: 'Official bank investor-relations PDF',
        issuer,
        ticker,
        documentType,
        documentTitle,
  ...
  return {
    provider: 'Official bank investor-relations PDF',
    issuer,
    ticker,
    sourceUrl,
    documentType,
    ...
    qualification: {
      status: errors.length ? 'UNVERIFIED' : 'QUALIFIED',
      extractionMethod: 'pre-extracted PDF text; no OCR performed by this qualifier',
      errors,
      page,
      table,
      documentVersion,
      noPeriodInference: true,
      noScopeInference: true,
      noUnitInference: true,
