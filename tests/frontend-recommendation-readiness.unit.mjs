...
const start = html.indexOf('function renderReadiness');
const end = html.indexOf('function render(d,tr)');
const readinessFn = html.slice(start, end);
assert.equal(/BUY|SELL/i.test(readinessFn), false, 'readiness renderer must not generate BUY/SELL');
