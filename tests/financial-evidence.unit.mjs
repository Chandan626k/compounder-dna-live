...
assert.equal(annual.reportingPeriod, '2025-03-31');
...
assert.notEqual(annual.evidenceId, make({ reportingPeriod: '2024-03-31' }).evidenceId, 'different reporting periods must not collide');
assert.equal(areEvidenceCompatible(annual, make({ sourceKey: 'equity', value: 1250 })), true);
assert.equal(areEvidenceCompatible(annual, make({ reportingDate: '2026-03-31T00:00:00.000Z', reportingPeriod: '2026-03-31' })), false);
assert.equal(areEvidenceCompatible(annual, make({ reportingPeriod: '2024-03-31' })), false);
