...
const snapshotWithoutPublication = createImmutableSourceSnapshot({ source: 'NSE_FIXTURE', sourceDocumentId: 'doc-no-time', retrievedAt: '2026-09-10T12:00:00Z', payload: { ok: true } });
assert.equal(assessHistoricalValidity(snapshotWithoutPublication, '2026-08-01T00:00:00Z').status, 'UNKNOWN');
...
assert.equal(resolved.find((r) => r.sourceDocumentId === 'doc-restatement').supersessionStatus, 'SUPERSEDES');
...
assert.throws(() => createReportingEvent({ ...base, sourceDocumentId: 'bad-rel', relationship: 'NEWER' }), /REPORTING_EVENT_RELATIONSHIP_INVALID/);
